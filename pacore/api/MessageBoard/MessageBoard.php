...
class MessageBoard {
  ...
  * @param id of message
  */
  static function delete_all_in_parent($parent_id,$parent_type) {
    Logger::log("Enter: function MessageBoard::delete_all_in_parent");
     if ($parent_type==PARENT_TYPE_MESSAGE) {
      ...
      $data = array($parent_id,$parent_type);
      Dal::query($sql, $data);
  ...
  * @param id of message
  */
  static function delete_replies($parent_id) {
    Logger::log("Enter: function MessageBoard::delete_replies");
    $sql = " DELETE FROM {boardmessages} WHERE parent_id = ? AND parent_type=? ";
    $data = array($parent_id,PARENT_TYPE_MESSAGE);
    Dal::query($sql, $data);
  ...
  */
  public function get($count = FALSE,$show='ALL',$page=0,$sort_by='changed',$direction='DESC') {
    ...
    if ($res->numRows() > 0) {
      ...
      while($row = $res->fetchRow(DB_FETCHMODE_OBJECT)) {
        ...
        if($user_value) {
          ...
          $arr_boardmessges[$i] = array('boardmessage_id' => $row->boardmessage_id,
                                'title' => $row->title, 
                                'body' => $row->body,
                                'created' => $row->created, 
    ...
    if ($res->numRows() > 0) {
      ...
      $row = $res->fetchRow(DB_FETCHMODE_OBJECT);
        $total_replies = $this->get_children_count($row->boardmessage_id,PARENT_TYPE_MESSAGE);
        $arr_boardmessges = array('boardmessage_id' => $row->boardmessage_id,
                              'title' => $row->title, 
                              'body' => $row->body,
                              'created' => $row->created, 
                              'total_replies' => $total_replies,
  ...
  */
  ...
  public function delete_user_forums( ) {
    ...
    if ($res->numRows() > 0) {
      while ($row = $res->fetchRow(DB_FETCHMODE_OBJECT)) {
        $pid = $row->boardmessage_id;
        MessageBoard::delete_replies($pid);//delete replies
        MessageBoard::delete($pid);//delete the board message itself
      }
    }  
