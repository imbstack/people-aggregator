...
class TypedGroupEntity extends Entity {
    ...
    public static function load_for_group($group_id, $load_entity = true) {
        ...
        if(!empty($result)) {
            ...
            $entity_result['entity_service'], $entity_result['entity_type'], $entity_result['entity_id']);
    ...
    public static function delete_for_group($group_id) {
        Entity::delete_match(array('entity_service' => 'typedGroup', 'entity_id' => $group_id,));
    }

    public static function get_count($entityType) {
        $result = Entity::search(array('entity_service' => 'typedGroup', 'entity_type' => $entityType,));
        return count($result);
    }

    public static function get_entities($entityType, $sort_by = null, $page = 1, $show = null) {
        $result = Entity::search(array('entity_service' => 'typedGroup', 'entity_type' => $entityType,), null, // attributes
        '=', // search method
        $sort_by, $page, $show);
        $entities = array();
