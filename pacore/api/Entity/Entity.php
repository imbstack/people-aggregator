...
class Entity extends DbObject {
    ...
    private static function create($e) {
        ...
        foreach($e as $k => $v) {
            ...
            if(empty($col)) {
                throw new Exception("Unknown entities table column '$k'");
            }
            $inserts[] = "$col=?";
            $args[] = $v;
        }
        ...
        foreach($entity_attributes as $ak => $d) {
			...
			SET id=?, attribute_name=?, attribute_value=?, attribute_permission=?", array($id, $ak, $ad['value'], @$ad['permission']));
        }
    }
    ...
    public function update($e) {
        ...
        foreach($entity_attributes as $ak => $d) {
            ...
            if(empty($this->attributes[$ak])) {
                // create it
                Dal::query("INSERT INTO {entityattributes} 
    		SET id=?, attribute_name=?, attribute_value=?, attribute_permission=?", array($this->id, $ak, $ad['value'], @$ad['permission']));
            }
            elseif($this->attributes[$ak] != $ad) {
                // it has changed
                Dal::query("UPDATE {entityattributes} SET attribute_value=?, attribute_permission=?
    		 WHERE id=? AND attribute_name=?", array($ad['value'], @$ad['permission'], $this->id, $ak));
            }
        ...
        foreach($this->attributes as $ak => $av) {
            switch($ak) {
                ...
                case 'modified':
                    continue;
                // these are ALWAYS to be retained
                break;
                default:
        ...
        // update cache
        $cache_key = "entity:$this->entity_service:$this->entity_type:$this->entity_id";
    ...
    public static function load_attributes($id) {
        ...
        while($row = Dal::row_assoc($r)) {
            $entity_attributes[$row['attribute_name']] = array(
    ...
    public static function delete_match($r) {
        ...
        foreach($entites as $entity) {
            // delete attributes
            Dal::query("DELETE FROM {entityattributes} WHERE id=?", array($entity->id));
            // delete the relation
            Dal::query("DELETE FROM {entities} WHERE id=?", array($entity->id));
        }
        return $entities;
    }
