...
class EntityRelation extends DbObject {
    ...
    public static function delete_match($r) {
        ...
        foreach($relations as $relation) {
            // delete attributes
            Dal::query("DELETE FROM {entityrelationattributes} WHERE id=?", array($relation->id));
            // delete the relation
            Dal::query("DELETE FROM {entityrelations} WHERE id=?", array($relation->id));
        }
        return $relations;
    }
    ...
    public static function load_attributes($id) {
        ...
        while($row = Dal::row_assoc($r)) {
            $relation_attributes[$row['attribute_name']] = array(
