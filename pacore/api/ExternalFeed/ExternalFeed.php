    ...
    public function import_posts() {
        ...
        if($feed_data) {
            ...
            for($counter = 0; $counter < $total_posts; $counter++) {
                ...
                $data = array(
                    $this->feed_id,
                    $feed_data[$counter]['title'],
                    $feed_data[$counter]['description'],
                    $feed_data[$counter]['original_url'],
                    $feed_data[$counter]['publish_date'],
                );
                try {
                    $res = Dal::query($sql, $data);
                }
        ...
        try {
            ...
            if($res->numRows()) {
                ...
                if($seconds_since_update-FEED_REFRESH_TIME > 0) {
                    $this->do_refresh = true;
                    //will be set when last_build_date is expired by more than FEED_REFRESH_TIME seconds
                }
                if($this->do_refresh) {
                    $feed_details = $this->get_feeds();
                    $this->import_url = $feed_details[0]->import_url;
                    //setting import url from feeds will be fetched.
                    if($this->import_url) {
                        $this->delete_user_feed_data();
                        //old feeds deleted for the import_url
                        $this->import_posts();
                        //fresh posts are being imported.
