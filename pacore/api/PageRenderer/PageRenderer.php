...
require_once "web/BlockModules/Module/Module.php";
// for PRI, HOMEPAGE constants
...
class PageRenderer {
    ...
    function __construct($cb, $page_id, $title, $page_template = "homepage_pa.tpl", $header_template = "header.tpl", $default_mode = PRI, $default_block_type = HOMEPAGE, $network_info_ = NULL, $onload = NULL, $setting_data = NULL) {
        ...
        elseif((false !== strpos($dynamic_page->page_type, 'user')) && (!empty(PA::$login_uid))) {
        ...
        if(!$access_permission) {
            ...
            elseif((!isset($configure['show_splash_page'])) || $configure['show_splash_page'] == INACTIVE) {
    ...
    private function preInitialize($setting_data) {
        ...
        if(!empty($setting_data['boot_code'])) {
            if(false == eval($setting_data['boot_code']."return true;")) {
                echo "<b>Evaluation of page boot code for page ID=".$setting_data['page_id']." failed"."Please check your  \"page_".$setting_data['page_id'].".xml\"  configuration file.";
            }
        }
        ...
        foreach(array("toprow", "middle", "left", "right") as $module_column) {
            ...
            if(count($column_modules) > 0) {
                foreach($column_modules as $moduleName) {
                    ...
                    $obj = new $moduleName;
                    $obj->login_uid = (int) PA::$login_uid;
                    ...
                    $obj->renderer = &$this;
                    // dispatch page renderer object to all modules
                    $obj->controller = &$app;
                    // dispatch front controller object to all modules
                    $obj->dynamic_page = &$dynamic_page;
                    // dispatch DynamicPage object to all modules
    ...
    private function get_extra_head_html() {
        ...
        if(!empty($extra['basic']['background_image']['name'])) {
            $extra_head_html .= "<style>body {background-image:url(".PA::$url."/files/".trim($extra['basic']['background_image']['name']).") !important;"// . "	background-repeat: no-repeat;"."}\n"// TODO: split the following CSS out into a css file
            // and @import it."#footer {color: #aaaaaa;}\n"."#container, #content, #col_a, #col_b, #col_c {
              background-image: url(none);
              background-color: transparent !important;
              }\n"// , #col_d."#AdvertisementModule {
              border: none;
              background-image: url(none);
              background-color: transparent !important;
