...
class Album extends ContentCollection {
   ...
   */
   ...
   public function load_first($user_id, $album_type) {
     ...
     if (empty($r)) throw new PAException(CONTENT_COLLECTION_NOT_FOUND, "No albums of type $album_type found");
   ...
   */
   ...
   static function album_exists ($author_id, $album_title, $album_type) {
    ...
    $data = array($author_id, $album_title, $album_type, 1);
    $res = Dal::query($sql, $data);
   ...
   * Function to find an Album by name. 
   Image, Audio and Video Album    type can have album of the same name.
      ...
      @param varchar $album_title.
   */
  static function find_album($author_id, $album_title, $album_type) {
  ...
  */
  ...
  public static function delete_user_albums( $user_id ) {
    ...
    $data = array( DELETED, ALBUM_COLLECTION_TYPE, $user_id );
    Dal::query( $sql, $data );
  ...
  public static function get_or_create_default($author_id, $alb_type) {
    ...
    try {
      ...
      if ($e->getCode() != CONTENT_COLLECTION_NOT_FOUND) throw $e;
