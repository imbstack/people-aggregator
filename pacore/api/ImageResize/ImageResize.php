...
define("RESIZE_STRETCH", 1);
// ignore aspect ratio, just stretch to the given size (output image will be exactly the desired size)
define("RESIZE_CROP", 2);
// scale the image to be larger than the output size, then crop out the middle (output image will be exactly the desired size)
define("RESIZE_CROP_NO_EXPAND", 3);
// as with RESIZE_CROP, but just crop to fit, don't scale up.
define("RESIZE_CROP_NO_SCALE", 6);
// as with RESIZE_CROP but don't scale at all; just cut the center out of the image if it's too big.
define("RESIZE_FIT", 4);
// scale the image to fit insize the output size (output image may be smaller than the desired size)
define("RESIZE_FIT_NO_EXPAND", 5);
// as with RESIZE_FIT, but only ever scale down.
...
*/
class ImageResize {
    static $skip_gd = FALSE;
    // set to TRUE to *not* use GD
    static $skip_magick = FALSE;
    // set to TRUE to *not* use ImageMagick
    ...
    */
    public static function resize_mk_img($root_path, // path to which all other paths passed to resize_mk_img are relative
    ...
    public static function resize_img($root_path, // path to which all other paths passed to resize_mk_img are relative
    $root_url, // url of $root_path
    $output_path, // relative path to a directory where we can put resized images
    $max_x, // max width of output image
    $max_y, // max height of output image
    ...
    $resize_type = RESIZE_CROP// RESIZE_FIT, RESIZE_CROP or RESIZE_STRETCH) {
        ...
        if($alternate) {
            if(preg_match("|^http://|", $alternate)) {
                throw new PAException(BAD_PARAMETER, "Alternate image passed to resizing functions must not be a URL");
            }
            if(!preg_match("#^(files|Themes|images)/#", $alternate)) {
                throw new PAException(BAD_PARAMETER, "Alternate image passed to resizing functions must be relative to the web directory; $alternate is not valid");
            }
        }
        if($picture instanceof StoredFile) {
            $stored_file = $picture;
            ...
            if(defined("NEW_STORAGE")) {
                ...
                if(preg_match("|^files/files|", $picture)) {
                    throw new PAException(INVALID_ID, "Broken image ID - starting with files/files!");
                }
                if(preg_match("|^files/pa://|", $picture)) {
                    throw new PAException(INVALID_ID, "Broken image ID - check for code adding 'files/' to the start of a pa:// image URL");
                }
            }
            $stored_file = NULL;
            $image_path = NULL;
            ...
            elseif(@getimagesize(PA::$core_dir."/$root_path/$picture")) {
                $image_path = PA::$core_dir."/$root_path";
            }
            elseif(@getimagesize(PA::$project_dir."/$root_path/$alternate")) {
                $image_path = PA::$project_dir."/$root_path";
            }
            elseif(@getimagesize(PA::$core_dir."/$root_path/$alternate")) {
                $image_path = PA::$core_dir."/$root_path";
            }
            if($picture && is_file("$image_path/$picture") && (getimagesize("$image_path/$picture") !== false)) {
                $pic_path = $picture;
            }
            elseif(!$alternate || !is_file("$image_path/$alternate")) {
    ...
    */
    ...
    public static function get_engine() {
        if(ImageResize::find_magick() && !ImageResize::$skip_magick) {
            return "magick";
        }
    ...
    public static function magick_resize_image($convert, $src_fn, $resized_fn, $max_x, $max_y, $resize_type = RESIZE_CROP) {
        ...
        switch($resize_type) {
            ...
            case RESIZE_CROP_NO_SCALE:
                ...
                if($resize_type == RESIZE_CROP_NO_SCALE) {
                    $scale = 1.0;
                }
        ...
        system($cmd, $retval);
        if($retval) {
            throw new PAException(GENERAL_SOME_ERROR, "$convert returned error code $retval (for command: $cmd)");
        }
    }
