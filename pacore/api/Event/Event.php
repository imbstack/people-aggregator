...
class Event extends Content {
  ...
  */
  public function save() {
    ...
    if (! $this->event_id) {
      ...
      */
      ...
      $sql = "UPDATE {events} SET "
        . "event_title = ?, start_time = ?, end_time = ?, event_data = ? "
        . "WHERE event_id = ?";
      $data = array(
        $this->event_title,
        $this->start_time,
    ...
    try {
      Dal::query($sql, $data);
      if (! $this->event_id) { // newly created
  ...
  public function load($cid) { 
    parent::load($cid); // the content part
    $this->load_by_event_id(Event::get_eid_from_cid($cid)); // the event part
  }
