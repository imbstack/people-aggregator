...
class Review {
  ...
  public static $columns = array("review_id", "is_active", "subject_type", "subject_id", "author_id", "title", "body", "created", "updated");
  ...
  function load($review_id) {
    ...
    if (empty($row)) throw new PAException(CONTENT_NOT_FOUND, "Could not find review id $review_id");
  ...
  function save() {
    ...
    if ($is_new) {
      if (!isset($this->is_active)) $this->is_active = 1;
    }
    ...
    foreach (Review::$columns as $col) {
      switch ($col) {
      case 'review_id': // never set
	continue;

      case 'updated': // always set automatically
	$set_fragments[] = "$col=NOW()";
	break;

      case 'created': // set automatically if new, otherwise copy
	if ($is_new) {
	  $set_fragments[] = "$col=NOW()";
	  break;
	} // else fallthrough
