...
class Normalizer extends XSLTProcessor {
    ...
    function __construct($inputFormat) {
        ...
        elseif(file_exists(PA::$core_dir."/api/ProfileIO/xsl/".$inputFormat.".xsl")) {
        ...
        if($xslfile) {
            ...
            throw new PAException(FILE_NOT_FOUND, "couldn't read tranformation stylesheet: $xslfile");
        }
...
class ProfileMerger {
    ...
    function __construct($user, $addProfileDOM) {
        ...
        // load the existing profile
        $this->currentProfileDOM = $this->loadDOM();
    ...
    public function diff() {
        ...
        foreach($diffProf->field as $field) {
            ...
            if($cpa) {
                ...
                $field['oldvalue'] = htmlspecialchars((string) $cpa[0]['value']);
                // we need to preserve the permissions!
                $field['perm'] = (int) $cpa[0]['perm'];
            }
            else {
                $field['perm'] = 1;
                // default permission
    ...
    public function loadDOM() {
        ...
        foreach($this->user as $k => $v) {
            if(in_array($k, $this->coreFields)) {
                ...
                $node->setAttribute('perm', 1);
                // always public
                $profile->appendChild($node);
            }
            elseif(in_array($k, array_keys($this->userSections)) && is_array($v)) {
                foreach($v as $f => $fd) {
            ...
            elseif(preg_match('/_relations/', $k) && is_array($v)) {
                foreach($v as $r) {
                    $node = $pdom->createElement("relation");
                    $node->setAttribute('network', ($r['network']) ? $r['network'] : 'internal');
