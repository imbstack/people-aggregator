...
class MSLiveDataMapper implements DataMapper {
    ...
    private static $email_type = 'EmailType';
    // Personal/Business
    public static $mslive_email_keys = array(
        'Business' => array(
            'Address' => 'professional|email'//     'IsDefault',
        ),
        'Personal' => array(
            'Address' => 'personal|email'//     'IsDefault',
        ),
        'Other' => array(
            'Address' => 'personal|email'//     'IsDefault',
        ),
    ...
    private static $phone_type = 'PhoneType';
    // Personal/Business
    public static $mslive_phone_keys = array(
        'Personal' => array(
            'Number' => 'general|phone'//     'IsDefault',
        ),
        'Business' => array(
            'Number' => 'general|phone'//     'IsDefault',
        ),
    ...
    public static $mslive_location_keys = array(
        'Business' => array(
            ...
            'CountryRegion',
            'Latitude',
            'Longitude'//     'Locations|Location|IsDefault',
        ),
        'Personal' => array(
    ...
    private static $uri_type = 'URIType';
    // Personal/Business
    ...
    public static function processInData($data = array()) {
        $root_paths = array(
            'Profiles|Personal' => array(
                'keys' => self::$mslive_pers_profile_keys,
                'type_field' => null,
            ),
            'Profiles|Professional' => array(
                'keys' => self::$mslive_prof_profile_keys,
                'type_field' => null,
            ),
            'Emails|Email' => array(
                'keys' => self::$mslive_email_keys,
                'type_field' => self::$email_type,
            ),
            'Phones|Phone' => array(
                'keys' => self::$mslive_phone_keys,
                'type_field' => self::$phone_type,
            ),
            'Locations|Location' => array(
                'keys' => self::$mslive_location_keys,
                'type_field' => self::$location_type,
            ),
            'URIs|URI' => array(
                'keys' => self::$mslive_uri_keys,
                'type_field' => self::$uri_type,
            ),
        );
        $result = array();
                    ...
                    foreach($map_table as $key1 => $key2) {
                        if(is_numeric($key1)) {
                            ...
                            if(isset($item[$key1]) && (!empty($key2))) {
                                ...
                                $section = $path[0];
                                $field = $path[1];
                                if(empty($item[$key1]) && (!empty($result[$section][$field]))) {
                                    continue;
                                }
