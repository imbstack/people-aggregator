...
//--------------- CRUD METHODS ----------------------------- //
    ...
    public  function save_PaForumThread( ) {
        ...
        if(!empty($this->id)) { 
          $itemsToUpdate = array('related_content_id' => $this->related_content_id,
                                 'title' => $this->title,
          ...
          $this->insert_PaForumThread($this->related_content_id,
                                      $this->title,
                                      $this->content,
                                      $this->status,
                                      $this->forum_id,
    ...
    public static function delete_ThreadsForUser( $user_id ) {
        ...
        foreach($threads as $thread) {             // when deleting a thread - all thread posts should be deleted
    ...
    public static function insertPaForumThread( $params = array() ) {
        ...
        $db_fields = array("related_content_id",
                           "title",
                           "content",
                           "status",
                           "forum_id",
        ...
        // call dynamic method 
        return $instance->insert_PaForumThread($related_content_id,
                                               $title,
                                               $content,
                                               $status,
                                               $forum_id,
      ...
      if(!is_null($page)) {
        if((string)$page == 'last') {
          ...
          if(isset($pages_arr['last'])) {
            $page = $pages_arr['last'];
          } else if(isset($pages_arr['pages'])) {
            $page = array_pop($pages_arr['pages']);
          }
        } else if((string)$page == 'first') {
          $page = 0;
          $this->pagination = new MemoryPagging($total_posts, $pagesize, $page);
    ...
    public function getThreadStatistics() {
      ...
      $statistics['last_post'] = $last_post; // (!empty($posts[0])) ? $posts[0] : null;
      return $statistics;
    }
