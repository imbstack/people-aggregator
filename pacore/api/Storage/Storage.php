...
class Storage {
    ...
    public static function save($current_filename, $filename, $file_class = "critical", $mime_type = "application/octet-stream", $meta = NULL) {
        // validate $file_class
        if(!in_array($file_class, array("critical", "throwaway"))) {
            throw new PAException(INVALID_ID, "Invalid file class $file_class");
        }
    ...
    public static function link($file_id, $params) {
        ...
        if(in_array("network", $required_params) && empty($params['network'])) {
            $params['network'] = PA::$network_info->network_id;
        }
            ...
            switch($param_name) {
                ...
                case "dim":
                    if(!preg_match("/^.*?\-\d+x\d+$/", $value)) {
                        throw new PAException(BAD_PARAMETER, "'dim' parameter must be of the format <prefix>-<width>x<height>");
                    }
                    break;
                case "file":
    ...
    public static function unlink($file_id, $link_id, $delete_immediately = TRUE) {
        ...
        if(empty($r)) {
            throw new PAException(FILE_NOT_FOUND, "Link $link_id does not exist");
        }
        // Find file and verify link count
        list($incomplete, $link_count) = Dal::query_one("SELECT incomplete, link_count FROM files WHERE file_id=?", array($file_id));
        if($incomplete) {
            throw new PAException(STORAGE_ERROR, "Consistency error... we have a link $link_id to an incomplete file $file_id");
        }
        if($link_count < 1) {
            throw new PAException(STORAGE_ERROR, "Consistency error... we have a link $link_id to a file $file_id, which claims to have no links");
        }
        // Delete the link
    ...
    public static function parseId($file_id, $allow_numeric = FALSE) {
        if($allow_numeric && preg_match("/^\d+$/", $file_id)) {
            return (int) $file_id;
        }
        if(preg_match("|^pa://(\d+)|", $file_id, $m)) {
            return (int) $m[1];
        }
    ...
    public static function validateFileId($file_id) {
        ...
        if($files && $path && strpos($path, $files) === 0) {
            return $file_id;
        }
        // otherwise fail
        return NULL;
    ...
    public static function getPath($file_id) {
        if(preg_match("|^http://|", $file_id)) {
            throw new PAException(INVALID_ID, "Trying to get the pathname of a URL");
        }
    ...
    public static function get_or_make_static($path, $mime_type = "application/octet-stream") {
        if(!$path) {
            throw new PAException(INVALID_ID, "Storage::get_or_make_static() requires a filename");
        }
        if(!file_exists(PA::$project_dir."/web/$path")) {
            throw new PAException(FILE_NOT_FOUND, "File $path does not exist");
        }
        ...
        if(strlen($path) > 255) {
            throw new PAException(INVALID_ID, "Path of file to store in fixed storage is too long - max 255 chars ($path)");
        }
    ...
    public static function cleanupFiles($min_age = 1800, $undelete_grace_period = 1800) {
        ...
        $sth = Dal::query("SELECT file_id FROM files WHERE link_count=0 AND last_unlinked IS NULL AND created < DATE_SUB(NOW(), INTERVAL ? SECOND)
            UNION SELECT file_id FROM files WHERE link_count=0 AND last_unlinked < DATE_SUB(NOW(), INTERVAL ? SECOND)", array($min_age, $undelete_grace_period));
    ...
    * doing! */
    ...
    public static function cleanupFile($file_id) {
        ...
        if(empty($r)) {
            throw new PAException(FILE_NOT_FOUND, "Cannot clean up file $file_id as it does not exist in the 'files' table");
        }
    ...
    public static function connect($backend_type = NULL) {
        if(empty($backend_type)) {
            $backend_type = PA::$config->storage_backend;
        }
    ...
    public static function validateFileType($path, $filename, $mime_type) {
        ...
        if(count($filename_bits) == 1) {
            ...
            $file_ext = array_pop($filename_bits);
        }
        ...
        if($mime_major == "image") {
            ...
            if(!$sc) {
                throw new PAException(INVALID_FILE, "Invalid image file");
            }
