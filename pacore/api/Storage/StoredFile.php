...
class StoredFile {
    ...
    public function getPath() {
        ...
        if(!$path) {
            throw new PAException(INVALID_ID, "Missing filename in StoredFile with ID ".$this->file_id);
        }
        if(!file_exists($path)) {
            throw new PAException(FILE_NOT_FOUND, "StoredFile with ID ".$this->file_id." ($path) does not exist");
        }
        return $path;
    }
