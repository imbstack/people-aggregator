...
<p>Currently, either the database doesn't exist, the user doesn't exist, or the password in <code>$peepagg_dsn</code> is incorrect.</p>
   ...
   <?php
    ...
    public static function register_query_callback($cb) {
        if(!Dal::$_query_callbacks) {
            Dal::$_query_callbacks = array();
        }
    ...
    public static function unregister_query_callback($cb) {
        ...
        if(!count(Dal::$_query_callbacks)) {
            Dal::$_query_callbacks = NULL;
        }
    }
    ...
    private static function execute_post_hooks($sql, $args) {
        ...
        if(Dal::$_query_callbacks) {
            foreach(Dal::$_query_callbacks as $cb) {
    ...
    public static function query($sql, $args = NULL) {
        ...
        if(PEAR::isError($sth)) {
            Logger::log(" Throwing exception DB_QUERY_FAILED | Message: ".$sth->getMessage()." | SQL that caused this exception: ".$sql, LOGGER_ERROR);
    ...
    public static function query_multiple($sql, $args = array(
        NULL,
    )) {
        global $query_count_on_page;
        $db  = Dal::get_connection();
    ...
    public static function execute_multiple($sql, $args = array(
        NULL,
    )) {
        global $query_count_on_page;
        $db  = Dal::get_connection();
    ...
    */
    public static function translate_table_name($table, $net_name = NULL) {
        ...
        if(!$db_table_mappings) {
            die("Internal error - $db_table_mappings is not available");
        }
        $table = trim($table);
        if(!$net_name) {
        ...
        if(!$pattern) {
            $pattern = @$db_table_mappings[$table];
        }
    ...
    public static function validate_sql($sql, $net_name = "") {
        return preg_replace("/\{\s*([a-z_]+)\s*\}/e", 'Dal::translate_table_name("", "$net_name")', $sql);
    }
