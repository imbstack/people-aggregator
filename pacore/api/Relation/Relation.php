...
class Relation {
  ...
  */
  ...
  public static function getRelationData($user_id, $relation_id, $network_id) {
    $res = Dal::query_one_object("SELECT * FROM {relations} WHERE user_id=? AND relation_id=? AND network_uid=?",
                                array($user_id, $relation_id, $network_id));
    if (!$res) return NULL;
    return new RelationData($res);
  }

  public static function add_relation(
    $user_id, $relation_id, $relation_type_id = 2,
    $network=NULL, $network_uid=NULL, $display_name=NULL,
    $thumbnail_url=NULL, $profile_url=NULL, $in_family = null, $status = APPROVED
    ...
    try {
      ...
      $sql = 'INSERT into {relations}
        ...
        values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)';
      $data = array(
        $user_id, $relation_id, $relation_type_id,
        $network, $network_uid, $display_name,
        $thumbnail_url, $profile_url, $in_family, $status
      );
      Dal::query($sql, $data);
  ...
  */
  public static function get_relation_record($user_id, $relation_id) {
    $rel = Dal::query_one_assoc("SELECT * FROM {relations} WHERE user_id=? AND ".(is_numeric($relation_id) ? 'relation_id' : 'profile_url')."=?",
        Array($user_id, $relation_id));
    if (!$rel) return NULL;
  ...
  */
  ...
  public static function get_relations($user_id, $status = NULL, $network_uid = NULL) {
    ...
    $data = array($user_id);
    $temp_data = array();
    if (!empty($status)) { // if status is specified
      $sql .= " AND status = ?";
      $temp_data = array($status);
    }
    $data = array_merge($data, $temp_data);
    $temp_data = array();
    if (!empty($network_uid)) { // if network_uid is specified
      $sql .= " AND network_uid = ?";
      $temp_data = array($network_uid);
    }
    $data = array_merge($data, $temp_data);
  ...
  public static function get_relations_detail($user_id) {
    ...
    while($row = Dal::row($res)) {
      ...
      $relations[]= array(intval($friend_id), intval($rel_type));
    }
  ...
  public static function lookup_relation_type_id($rel_type) {
      ...
      if (!$r) throw new PAException(INVALID_RELATION, "Unknown relation type '$rel_type'");
      return intval($r[0]);
  }
  ...
  public static function lookup_relation_type($rel_type_id) {
      ...
      if (!$r) throw new PAException(INVALID_RELATION, "Unknown relation type ID $rel_type_id");
      return $r[0];
  }
  ...
  */
  static function get_relations_of_type($user_id, $type, $pagesize=0, $page_no=1) {
    Logger::log("Enter: Relation::get_relation_type");
    $user_relation_data = array();
    $i = 0;
    $count1 = ($page_no - 1) * $pagesize;
    $count2 = $pagesize;   // TO DO: Calculate $count2 = $pagesize-1 or $count2 = $pagesize
  ...
  */
  ...
  static function get_external_relations($user_id,
    $no_of_relations = 0, $cnt=FALSE, $show='ALL',
    $page=0, $sort_by='created', $direction='DESC'
    ) {
    // this is just a convenience wrapper
    return Relation::get_all_relations($user_id, $no_of_relations, $cnt, $show, $page, $sort_by, $direction,
    'external');
    }
  ...
  * @return $user_relation_data an array having user relation's information
  */
  static function get_all_relations($user_id,
    $no_of_relations = 0, $cnt=FALSE, $show='ALL',
    $page = 0, $sort_by = 'created', $direction = 'DESC',
    $scope='internal', $status = NULL, $network_uid = NULL) { // status added
    ...
    // relations inside one PA instance
      $sql = "SELECT *, R.relationship_type as Rel_id, RC.relation_type AS RT,
        ...
        AND U.is_active <> ".DELETED; // we don't want to see deleted users, do we?, omitting only deleted users (keeping the disabled ones)
    }
    $temp_data = array();
    if (!empty($status)) { // if status is specified
      $sql .= " AND R.status = ?";
    ...
    $temp_data = array();
    if (!empty($network_uid)) { // if status is specified
  ...
  */
  static function get_all_user_relations($my_user_id, $no_of_userids = 0, $cnt=FALSE, $show='ALL', $page=0, $sort_by='created', $direction='DESC', $scope='internal', $status = NULL, $network_uid = NULL) {
  ...
  public static function get_relation($user_id, $relation_id, $network_uid=NULL) {
    ...
    if (!$r) throw new PAException(RELATION_NOT_EXIST, "There is no relation between $user_id and $relation_id");
  ...
  */
  public static function delete_relation ($user_id, $relation_id, $network_uid = NULL, $network=NULL) {
    ...
    $data = array($user_id, $relation_id);
  ...
  * @return $user_data an array having user relation's information
  */
  static function get_all_user_ids($my_user_id, $no_of_userids = 0, $cnt=FALSE, $show='ALL', $page=0, $sort_by='created', $direction='DESC', $status = NULL, $network_uid = NULL) {
  ...
  */
  ...
  public static function delete_user_relations( $user_id ) {
    ...
    $data = array( '0'=>$user_id, '1'=>$user_id );
...
// find whether relation between two given users already exists
  ...
  */
  public static function update_relation_status($user_id, $relation_id, $status, $network_uid = null) {
    ...
    $data = array($status, $user_id, $relation_id);
    if($network_uid) {
      $sql .= " AND network_uid = ?";
      array_push($data, $network_uid);
    }
  ...
  public static function update_relation_network_info($user_id, $relation_id, $network, $network_uid) {
    ...
    $data = array($network, $network_uid, $relation_id, $user_id);
    try {
      $res = Dal::query($sql, $data);
      if ($res) {
        $return = true;
...
*/
  public static function get_relation_ids($login_id, $params = NULL, $time_interval =
                                          NULL) {
