    ...
    **/
    ...
    private function _can_user($uid, $target_id, $target_type, $strict, $params) {
        ...
        if(false !== strpos($params['permissions'], ',')) {
            $_strict = true;
            // permissions separated by comas - that means 'must have all required permiss.'
            $required_permissions = explode(',', $params['permissions']);
        }
        elseif(false !== strpos($params['permissions'], '|')) {
            $_strict = false;
            // permissions separated by 'OR' - that means 'must have one of required permiss.'
            $required_permissions = explode('|', $params['permissions']);
        }
        else {
            $_strict = true;
            // only one permission - that means 'must have this required permiss.'
    ...
    private function can_delete_comment($params, $type) {
        ...
        if(!empty($params['comment_info'])) {
            ...
            elseif(isset($comment['recipient_id']) and ($comment['recipient_id'] == $this->uid)) {
                return true;
            }
            ...
            elseif($content->parent_collection_id !=-1) {
    ...
    private function can_delete_comment_authorization($params, $type) {
        ...
        $perm_array = array(
            @$params['group_owner'],
            $params['content_owner'],
            $params['comment_owner'],
        );
