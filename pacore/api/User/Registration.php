    ...
    */
    function register($params, $network_info = NULL) {
        ...
        if(!$this->error) {
            ...
            $this->array_of_errors = array(
                "error_login"         => @$error_login,
                "error_first_name"    => @$error_first_name,
                "error_email"         => @$error_email,
                "error_password_conf" => @$error_password_conf,
                "error_password_l"    => @$error_password_l,
                "error_password_g"    => @$error_password_g,
            );
        }
        if($this->error != TRUE) {
        ...
        if($this->error != TRUE) {
            try {
                ...
                if($mother_extra['email_validation'] == NET_NO) {
                ...
                $data_array = array(
                    array(
                        'uid'   => $this->newuser->user_id,
                        'name'  => 'first_name',
                        'value' => $this->newuser->first_name,
                        'type'  => BASIC,
                        'perm'  => 1,
                    ),
                    array(
    ...
    public static function add_default_links($user_id) {
        ...
        for($counter = 0; $counter < count($link_categories); $counter++) {
            $param_array = array(
                'category_name'                               => $link_categories[$counter]->category_name,
                'user_id'                                     => $user_id, 'created' => time(), 'changed' => time(),
                'is_active'                                   => ACTIVE,
            ); $Links->set_params($param_array); $category_id = $Links->save_category();
            ...
            $condition = array(
                'category_id' => $link_categories[$counter]->category_id,
                'is_active' => ACTIVE,
            );
            ...
            for($i = 0; $i < count($list_array); $i++) {
                $param_array = array(
                    'title'   => $list_array[$i]->title,
                    'url'     => $list_array[$i]->url, 'category_id' => $category_id, 'created' => time(),
                    'changed' => time(), 'is_active' => ACTIVE,
    ...
    public function add_default_header($user_id) {
        ...
        if(!empty($header_image)) {
            $new_profile['user_caption_image'] = array(
                "name" => "user_caption_image",
                "value" => $header_image,
            );
        }
        if(!empty($header_image_display)) {
            $new_profile['desktop_image_display'] = array(
                "name" => "desktop_image_display",
                "value" => $header_image_display,
            );
        }
        if(!empty($header_image_option)) {
            $new_profile['desktop_image_action'] = array(
                "name" => "desktop_image_action",
                "value" => $header_image_option,
            );
        }
        try {
            $user = new User();
