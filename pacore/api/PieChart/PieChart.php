...
class PieChart {
    ...
    */
    function PieChart($w = 200, $h = 150, $data) {
    ...
    /**
    *	onverts Hexcolors to RGB
    *@returns array
    */
    function hex2rgb($hex) {
        $color = ereg_replace('^#', '', $hex);
        $rgb = array((16*hexdec(substr($color,
            0,
            1))+hexdec(substr($color,
            1,
            1))),(16*hexdec(substr($color,
            2,
            1))+hexdec(substr($color,
            3,
            1))),(16*hexdec(substr($color,
	...
	*/
    function init() {
        ...
        $this->threedee_height = ($this->threedee_height) ? $this->threedee_height : 10;
        //round($this->pie_width/10);
    ...
    function get_color($num, $mode = "normal") {
        ...
        $tmp_color_3d = array((($tmp_color[0] > 79) ? $tmp_color[0]-80 : 0),(($tmp_color[1] > 79) ? $tmp_color[1]-80 : 0),(($tmp_color[2] > 79) ? $tmp_color[2]-80 : 0),
        );
        if($mode == "3d") {
    ...
    function display() {
        ...
        for($i = $this->start_3d; $i > $this->cy; $i--) {
            ...
            foreach($this->pie_data as $k => $data) {
                ...
                ImageFilledArc($this->pie, $this->cx, $i, $this->init_width, $this->init_height, $data[0], $data[1], $col, IMG_ARC_NOFILL);
                $c++;
            }
        }
        // Now do the graph
        ...
        foreach($this->pie_data as $k => $data) {
            ...
            ImageFilledArc($this->pie, $this->cx, $this->cy, $this->init_width, $this->init_height, $data[0], $data[1], $col, IMG_ARC_PIE);
            $c++;
        }
        // The Legends
        $cellpadding = 5;
        ...
        ImageCopyResampled($final, $this->pie, 0, 0, 0, 0, $this->img_width, $this->img_height, $pie_width, $pie_height);
        ImageCopyResampled($final, $leg_img, $this->img_width+$cellpadding, 0, 0, 0, $leg_width, $leg_height, $leg_width, $leg_height);
