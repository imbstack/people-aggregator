...
class InputSanitizer extends DomDocument {
    ...
    public $dropWithChildren = Array(
        "head",
        "script",
        "frameset",
    );

    public $elemWhitelist = Array(
        "html",
        "style",
        "h1",
        "h2",
    ...
    public $attrValueBlacklist = Array(
...
<meta http-equiv="Content-Type" content="text/html; charset=utf-8"/>
        ...
        elseif(is_array($input)) {
            foreach($input as $key => $value) {
                ...
                elseif(!$this->htmlAllowedEverywhere && !@$this->tags_allowed_in_fields[$key]) {
                    $t = strip_tags($value);
                    if($length) {
                        // truncate string
    ...
    function dump_nodes($node = null, $indent = null, $prevcharcount = 0) {
        ...
        if($node->hasChildNodes()) {
            foreach($node->childNodes as $child) {
                if($child->nodeName == "#text") {
                    ...
                    if($this->maxchars > 0) {
                        ...
                        for($tpos = 0; $tpos < $tlen && ($charcount < $this->maxchars); ++$charcount) {
                            ...
                            $char_len = (!($c&0x80)) ? 1 : ((($c&0xe0) == 0xc0) ? 2 : ((($c&0xf0) == 0xe0) ? 3 : ((($c&0xf8) == 0xf0) ? 4 :-1)));
                            if($char_len ==-1) {
                                // invalid utf-8 - fall back to byte-by-byte counting
                                ++$tpos;
                            }
                            else {
                                // character size detected
                                $tpos += $char_len;
                            }
                        }
                ...
                elseif(in_array($child->nodeName, $dropWithChildren)) {
                    // we do nothing but indent and prettyprint here
                    $html .= $this->doElemWrap();
                }
                elseif(!in_array($child->nodeName, $elemWhitelist)) {
