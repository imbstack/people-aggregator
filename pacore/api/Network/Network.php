   ...
   */
    ...
    */
    ...
    public function is_private() {
        if(PA::$config->all_networks_are_private) {
            return TRUE;
        }
        if($this->type == PRIVATE_NETWORK_TYPE) {
            return TRUE;
        }
        return FALSE;
    }
    ...
    */
    ...
    public static function get_network_by_address($network_name) {
        if($network_name == 'www' || empty($network_name)) {
            return self::get_mothership_info();
        }
    ...
    */
    ...
    function get_user_networks($uid, $params = NULL) {
        ...
        $network = array();
        while($row = $res->fetchRow(DB_FETCHMODE_OBJECT)) {
            $row->members = $row->member_count;
            // alias for old code that expects 'members' rather than 'member_count'
            $network[] = $row;
        }
    ...
    */
    ...
    function get_network_members($nid, $params = NULL, $conditions = NULL) {
        Logger::log("[ Enter: function Network::get_user_networks] \n");
        ...
        if($conditions['keyword']) {
            ...
            //if no keyword is specified but owner is viewing network user
    ...
    */
    ...
    function join($network_id, $uid, $user_type = null, $by_admin = false) {
    ...
    */
    ...
    function leave_all_networks($uid) {
    ...
    function update_network_member_count($network_id) {
    ...
    */
    ...
    function member_exists($network_id, $uid) {
    ...
    */
    ...
    function is_admin($network_id, $uid) {
        ...
        */
        if($is_super_user) {
            // SUPER USER for spawned network
            Logger::log("Exit: static function Network::is_admin | Return: TRUE");
            return true;
        }
        ...
        if($row->user_type == NETWORK_OWNER) {
            // NETWORK OWNER for spawned network
            Logger::log("Exit: static function Network::is_admin | Return: TRUE");
            return true;
        }
        else {
    ...
    */
    ...
    function check_already($address) {
    ...
    */
    ...
    function get_user_type($network_id, $uid) {
    ...
    */
    public function do_network_setup() {
        ...
        elseif(file_exists(PA::$core_dir.'api/DB/PeepAgg_tmpl.mysql')) {
    ...
    */
    public static function get_by_id($network_id) {
        ...
        */
        ...
        $row                = $res->fetchRow(DB_FETCHMODE_OBJECT);
        $row->total_members = $row->member_count;
        // alias for old code that expects to see 'total_members' rather than 'member_count'
    ...
    */
    public static function get_network_owner($network_id) {
        ...
        if(!$row) {
            ...
            return NULL;
            // No owner - bad, but known to happen on early early code!
        }
        if($row->owner_id == 0) {
            // required for compability with earlier PA versions
    ...
    public static function get_members($params) {
        ...
        if($params['network_id'] == 1) {
            ...
            if($owner_arr['total_users'] == 0) {
                Network::update_membership_type(array('user_id_array' => array(
                    1,
                ), 'user_type' => NETWORK_OWNER, 'network_id' => 1));
            }
        }
        $data = array();
        $notshow = UNVERIFIED;
