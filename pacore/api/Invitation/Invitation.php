   ...
   public function send() {
    ...
    public function deny() {
      ...
      if ($res->numRows()) {
        ...
        $array_of_data = array('first_name'=>$user->first_name , 'last_name'=>$user->last_name, 'user_name'=> $user->login_name, 'user_id'=>$user->user_id, 'group_name'=> $this->inv_group_name);
      }
    ...
    public static function get_all($user_id, $collection_id = -1) {
      ...
      while ($row = $res->fetchRow(DB_FETCHMODE_OBJECT)) {
        $invites[] = array('id' => $row->inv_id, 'status' => $row->inv_status);
      }
...
inv_collection_id = ? ", array($GInvID, $collection_id));
    ...
    */
    public static function validate_invitation_id ($invitation_id) {
      ...
      $data = array($invitation_id, INVITATION_PENDING);
