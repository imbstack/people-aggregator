...
class Roles {
    ...
    */
     ...
     *
     */
    public static function delete_user_roles($user_id, $roles_info =-1, $group_id = null) {
        // if($role_id == -1) all user roles will be deleted !!!
        ...
        $user->load($user_id);
        if($roles_info ==-1) {
            // delete all user roles (default option)
    ...
    */
    ...
    public static function get_user_roles($user_id, $fetch_mode = DB_FETCHMODE_OBJECT, $params = null) {
        ...
        if($user_id) {
            ...
            // get ANONYMOUS_ROLE
            $r[0] = (object) array('user_id' => 0, 'role_id' => ANONYMOUS_ROLE, 'extra' => "a:3:{s:4:\"user\";b:0;s:7:\"network\";b:1;s:6:\"groups\";a:0:{}}");
        }
