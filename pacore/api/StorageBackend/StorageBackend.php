...
class StorageBackend {
    ...
    public function download($current_url, $file_id, $filename, $file_class, $mime_type) {
        ...
        try {
            ...
            curl_setopt_array($ch, array(CURLOPT_FAILONERROR => TRUE, CURLOPT_FOLLOWLOCATION => FALSE, CURLOPT_FORBID_REUSE => TRUE, CURLOPT_LOW_SPEED_LIMIT => 1024, // kill transfer if slower than 1kB/s for > 5 s
            CURLOPT_LOW_SPEED_TIME => 5, CURLOPT_TIMEOUT => 30, // kill transfer if it takes > 30 s
            CURLOPT_FILE => $temp_file,));
            $r = curl_exec($ch);
            fclose($temp_file);
            $temp_file = NULL;
            if(!$r) {
                throw new PAException(FILE_NOT_UPLOADED, sprintf(__("Failed to retrieve %s: %s"), $current_url, curl_error($ch)));
            }
            curl_close($ch);
            $ch = NULL;
