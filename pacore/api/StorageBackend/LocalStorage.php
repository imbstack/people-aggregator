...
class LocalStorage extends StorageBackend {
    ...
    public function save($current_path, $file_id, $filename, $file_class, $mime_type) {
        if(!file_exists($current_path)) {
            throw new PAException(FILE_NOT_FOUND, "File to store not found (at $current_path)");
        }
        $filename = Storage::validateFileType($current_path, $filename, $mime_type);
        //TODO: maybe move this inside Storage::save?
        $leaf = $this->makeFilename($file_id, $filename);
        $path = $this->getPath($leaf, FALSE);
        if(!@copy($current_path, $path)) {
            throw new PAException(STORAGE_ERROR, "Error copying $current_path to $path");
        }
        // make a note that we have a copy, so it will get replicated onto other servers
        Dal::query("INSERT INTO local_files SET file_id=?, timestamp=NOW(), filename=?, servers=?", array($file_id, $leaf, PA::$server_id));
    ...
    public function getURL($leaf) {
        $path = $this->getPath($leaf);
        if(!file_exists($path)) {
            throw new PAException(FILE_NOT_FOUND, "Unable to find file to generate url: $path");
        }
    ...
    public function getPath($leaf, $must_exist = TRUE) {
        ...
        if($must_exist && !file_exists($path)) {
            throw new PAException(FILE_NOT_FOUND, "File not found: $path");
        }
        return $path;
    }
