   ...
   */
    ...
    */
    ...
    public function join($user_id, $email = NULL, $GInvID = NULL) {
        ...
        switch($this->reg_type) {
            ...
            default:
                ...
                $role = array(
                    'role_id' => GROUP_MEMBER_ROLE,
                    'extra' => serialize($role_extra),
                );
    ...
    public static function get_members_with_roles($group_id) {
        ...
        while($row = $res->fetchRow(DB_FETCHMODE_OBJECT)) {
            ...
            foreach($u->role as $i => $role) {
                ...
                if(is_array($extra['groups'])) {
                    // we are only looking for group relevant roles
    ...
    public static function get_moderators($group_id) {
        ...
        foreach($members as $i => $member) {
            if(!empty($member->role_ids)) {
                if(in_array(GROUP_MODERATOR_ROLE, $member->role_ids)) {
                    $moderators[] = $member;
                }
            }
        }
    ...
    public static function get_admins($group_id) {
        ...
        foreach($members as $i => $member) {
            if(!empty($member->role_ids)) {
                if(in_array(GROUP_ADMIN_ROLE, $member->role_ids)) {
                    $admins[] = $member;
                }
            }
        }
        ...
        if($res->numRows()) {
            while($row = $res->fetchRow(DB_FETCHMODE_OBJECT)) {
                ...
                $groups[] = array(
                    ...
                    'title'     => $group_by_id->title,
                    'author_id' => $group_by_id->author_id,
                    'created'   => $group_by_id->created,
                    'members'   => $row->cnt,
                    'picture'   => $group_by_id->picture,
                );
            }
        }
    ...
    public static function is_admin($group_id, $user_id) {
        ...
        // how can an annon user be admin ^^
        $is_super_user = false;
        ...
        if($res->numRows() || $is_super_user) {
            // SUPER USER or GROUP OWNER!
    ...
    function load_groups_for_category($category_id, $cnt = FALSE, $show = 'ALL', $page = 0, $sort_by = 'created', $direction = 'DESC') {
    ...
    */
    ...
    function save_new_group($ccid, // content collection id
    ...
    $access = 0, $reg = 0, $is_mod = 0, $header_image = "", $header_image_action = NULL, $display_header_image = NULL, $extra = NULL) {
        if(empty($title)) {
    ...
    public function delete_user_groups($user_id, $group_id = NULL) {
        ...
        if(count($all_user_groups) > 0) {
            foreach($all_user_groups as $group) {
                ...
                elseif(($group['access'] == MEMBER) || ($group['access'] == MODERATOR)) {
    ...
    */
    public function get_group_theme_detail() {
        ...
        if($res->numRows()) {
            while($row = $res->fetchRow(DB_FETCHMODE_OBJECT)) {
                $groups_info = array(
                    'group_id'            => $this->collection_id,
                    'header_image'        => $row->header_image,
                    'header_image_action' => $row->header_image_action,
                    'extra'               => $row->extra,
                );
            }
        }
