...
define("SPAM_STATE_OK", 0);
// not spam
define("SPAM_STATE_MANUALLY_DELETED", 1);
// deleted manually in edit interface
define("SPAM_STATE_SPAM_WORDS", 2);
// deleted automatically due to presence of spam words
define("SPAM_STATE_AKISMET", 3);
// deleted automatically due to a postitive check-comment response from akismet.com
define("SPAM_STATE_DOMAIN_BLACKLIST", 4);
// deleted automatically due to a blacklisted domain (automatic or manual)
define("SPAM_STATE_TOO_MANY_LINKS", 5);
// deleted automatically due to excessive # of links
/**
...
class Comment {
    ...
    function count_comments_for_content($content_id) {
    ...
    function get_comment_for_content($content_id = NULL, $count = '', $order_by = 'DESC', $ignore_collection = NULL) {
...
COM.content_id=CON.content_id AND CON.is_active = ? AND COM.parent_type <> ? AND COM.parent_type <> ?";
    ...
    function delete_all_comment_of_content($content_id, $is_active = 0) {
    ...
    function get_comment_for_user($user_id = NULL, $count = 10) {
...
AND parent_type <> ? ORDER BY created DESC $limit";
    ...
    function count_all_comments($uid = 0) {
        if(!$uid) {
    ...
    */
    ...
    function get_all_comments($uid = 0, $show = 'ALL', $page = 1, $sort_by = 'created', $direction = 'DESC') {
        if($show == 'ALL') {
            $limit = "";
        }
        else {
        ...
        if(!$uid) {
            ...
            $sth = Dal::query("SELECT $select_fields
            FROM {comments} cmt
    ...
    */
    ...
    function get_selected($comment_ids) {
        return Dal::query_multiple("SELECT * FROM {comments} WHERE comment_id = ?", array_map(create_function('$id', 'return array($id);'), $comment_ids));
    }

    /* add a term into the spam_terms table */
    static

    function add_spam_term($term, $blacklist = 1) {
    ...
    function set_spam_state($comment_ids, $new_state) {
    ...
    */
    function get_links() {
        $links = array();
        if($this->homepage && $this->name) {
            $links[$this->homepage] = array(
                $this->name,
            );
        }
        if($this->comment) {
    ...
    private function spam_analyze() {
        if(!$this->comment_id) {
            throw new PAException(INVALID_ID, "Can't spam-analyze a comment without an ID");
        }
        $this->akismet_check();
