...
class Subversion_PAStateStore {
    ...
    */
    function initialize() {
        ...
        if(!$this->revision) {
            throw new Subversion_Failure("Could not find 'Revision: XXX' line in dist_files.txt");
        }
    ...
    */
    ...
    function update_object($path, $kind, $hash, $revision) {
        Dal::query("INSERT INTO svn_objects SET path=?, kind=?, hash=?, revision=?, is_active=1", array($path, $kind, $hash, $revision));
        Dal::query("UPDATE svn_objects SET is_active=0 WHERE is_active=1 AND path=? AND revision < ?", array($path, $revision));
    }
    ...
    function get_held_revision($path) {
        ...
        if(!$r) {
            throw new Subversion_Failure("Attempt to get held revision for path $path, which isn't in the database");
        }
        return $r[0];
    }
