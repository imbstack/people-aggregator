...
class Subversion_Svndiff {
    ...
    function apply($source = NULL) {
        if($this->debug) {
            echo "parsing txdelta: ".strlen($this->tx)." bytes\n";
        }
        $this->source = $source;
        $this->target = "";
        $this->len    = strlen($this->tx);
        $this->pos    = 0;
        if($this->getc() != 'S' || $this->getc() != 'V' || $this->getc() != 'N' || $this->getc() != 0) {
        ...
        if($this->pos != $this->len) {
            throw new Subversion_Failure("position mismatch after reading all windows: pos=$this->pos, but we should be at the end: len=$this->len");
        }
        return $this->target;
    }
    ...
    function getc() {
        if($this->pos >= $this->len) {
            throw new Subversion_Failure("run out of data in txdelta");
        }
    ...
    function get_data($len) {
        if($len > $this->len-$this->pos) {
            throw new Subversion_Failure("run out of data in txdelta (attempting to read $len bytes, but we only have ".($this->len-$this->pos)." left)");
        }
    ...
    function parse_window() {
        if($this->debug) {
            echo "parsing window at $this->pos: ";
        }
        ...
        if($this->debug) {
            echo "src view offset $src_view_offset, src view len $src_view_len, target view len $targ_view_len, instructions: $ins_len, data: $data_len\n";
        }
        $win_target = "";
        ...
        while($this->pos < $ins_end) {
            if($this->debug) {
                echo "[@".($this->pos-$ins_start)."/$ins_len] ";
            }
            $c        = ord($this->getc());
            $ins      = $c >> 6;
            $copy_len = $c&0x3f;
            if(!$copy_len) {
                $copy_len = $this->get_int();
            }
            if($this->debug) {
                $target_start = strlen($win_target);
            }
            switch($ins) {
                case 0:
                    $copy_ofs = $this->get_int();
                    if($this->debug) {
                        echo "copy from source: offset $copy_ofs, len $copy_len\n";
                    }
                    if($copy_ofs > strlen($this->source)) {
                        throw new Subversion_Failure("can't copy from past the end of the source; source len=".strlen($this->source)." but trying to copy from offset $copy_ofs");
                    }
                    if($copy_ofs+$copy_len > strlen($this->source)) {
                        throw new Subversion_Failure("copy region extends past the end of the source");
                    }
                ...
                case 1:
                    $copy_ofs = $this->get_int();
                    if($this->debug) {
                        echo "copy from target (offset $copy_ofs, len $copy_len)\n";
                    }
                    if($copy_ofs > strlen($win_target)) {
                        throw new Subversion_Failure("can't copy from past the end of the target");
                    }
                ...
                case 2:
                    if($this->debug) {
                        echo "copy from data (len $copy_len)\n";
                    }
                    if($copy_len > $data_end-$data_pos) {
                        throw new Subversion_Failure("can't copy $copy_len bytes from data as only ".($data_end-$data_pos)." are available");
                    }
            ...
            if($this->debug) {
                echo "wrote to target: [".substr($win_target, $target_start)."]\n";
            }
        }
        if($data_pos != $data_end) {
            throw new Subversion_Failure("not all data used: pos=$data_pos but end=$data_end");
        }
        // skip over data
        if($this->pos != $ins_end) {
            throw new Subversion_Failure("after reading instructions, we should be at pos $ins_end, but instead we're at $this->pos");
        }
        $this->pos += $data_len;
        if(strlen($win_target) != $targ_view_len) {
            throw new Subversion_Failure("we were meant to add $targ_view_len bytes to the target, but actually ".strlen($win_target)." bytes were added");
        }
...
class Subversion_StandaloneClient {
    ...
    function rename_to_dotlocal($full_path) {
        ...
        if(!rename($full_path, $new_path)) {
            throw new Subversion_Failure("Unable to rename $full_path to $new_path");
        }
        clearstatcache();
    ...
    function enter_dir($dn) {
        ...
        elseif(in_array($this->current_path, $this->skipped_dirs)) {
            ...
            if(!is_dir($this->current_path)) {
                throw new Subversion_Failure("Cannot enter directory: $this->current_path does not exist");
            }
        }
    }
        ...
        switch($tag) {
            ...
            case 'S:DELETE-ENTRY':
                ...
                elseif(is_dir($full_path)) {
                    ...
                    $desc_list = $this->state->find_descendants($fn);
                    // this always returns them in reverse order, i.e. safe deletion order
            ...
            break;
            case 'D:HREF' : $this->href = "";
            $this->reading_href = true;
            break;
        case 'S:TXDELTA' : if(!$this->current_file) {
            throw new Subversion_Failure("txdelta while not adding/opening file");
        }
        $this->txdelta = "";
        $this->reading_txdelta = true;
        break;
    case 'V:MD5-CHECKSUM' : $this->checksum = "";
    $this->reading_checksum = true;
    break;
case 'S:OPEN-DIRECTORY' : $dn = isset($attrs['NAME']) ? $attrs['NAME'] : '';
$this->enter_dir($dn);
break;
case 'S:ADD-DIRECTORY' : $dn = $attrs['NAME'];
...
elseif($dir_already_there) {
    ...
    if(!$this->dry_run) {
        if(!@mkdir($full_path)) {
            throw new Subversion_Failure("Failed to make dir $full_path.");
        }
        chmod($full_path, 0777);
        // in case the web server's umask stopped us creating it like that before
        clearstatcache();
...
$this->enter_dir($dn);
break;
case 'S:ADD-FILE' : $this->adding_file = true;
// fall through
case 'S:OPEN-FILE' : $this->current_file = $attrs['NAME'];
$this->checksum = null;
break;
}
}
...
function xml_close_tag($xp, $tag) {
    switch($tag) {
        ...
        case 'S:OPEN-FILE':
            ...
            if(!$this->checksum) {
                throw new Subversion_Failure("Updates is missing a checksum for file $full_path");
            }
            // A or U
            $op_display_as = "";
            ...
            try {
                ...
                if($this->adding_file) {
                    ...
                    // patching a file (U)
                    $op_display_as = "U";
                    $save_download_info = FALSE;
                    // TRUE if a force-apply will need to download this file
                ...
                if($is_download) {
                    ...
                    if(!file_exists($download_path)) {
                        throw new Subversion_Failure("$download_path has not been downloaded");
                    }
                    $new_content = file_get_contents($download_path);
                    $this->queue_delete($download_path);
                    if($new_content == FALSE) {
                        throw new Subversion_Failure("Unable to read $download_path");
                    }
                }
                elseif(!$this->has_recent_errors()) {
                    // if we got this far, we must have a patch for the file.
                    if(!$this->txdelta) {
                        throw new Subversion_Failure("Update is missing a text delta for file $full_path");
                    }
...
function xml_data($xp, $data) {
    ...
    elseif($this->reading_checksum) {
        $this->checksum .= $data;
    }
    elseif($this->reading_href) {
        $this->href .= $data;
    }
}
...
*/
function apply_patch($apply = FALSE) {
    //$skip=array(), $apply=FALSE) {
    if(!in_array($apply, array(FALSE, TRUE, "force", "soft"))) {
        throw new Subversion_Failure("illegal value for $apply: $apply");
    }
    ...
    try {
        ...
        if(!$f) {
            throw new Subversion_Failure("can't open $this->diff_fn");
        }
        while(!feof($f)) {
            $data = fread($f, 8192);
        ...
        if(!$this->dry_run) {
            ...
            foreach($this->rename_queue as $names) {
                ...
                if(file_exists($to_name)) {
                    ...
                    if(!$temp_name) {
                        throw new Subversion_Failure("Unable to make temporary file in ".dirname($to_name));
                    }
                ...
                if(!rename($from_name, $to_name)) {
                    throw new Subversion_Failure("Unable to rename $from_name to $to_name");
                }
    ...
    return array('target_revision' => $this->target_revision, 'errors' => $this->errors,);
}
...
function url_join($root, $rel) {
    ...
    if(!@$root_info['scheme']) {
        throw new Subversion_Failure("$root is not an absolute URL");
    }
