 ...
 * @version    CVS: $Id: oci8.php,v 1.104 2007/01/04 08:16:58 aharvey Exp $
        ...
        foreach($tokens as $key => $val) {
            switch($val) {
                ...
                case '!':
                    ...
                    break;
                default:
                    $tokens[$key] = preg_replace('/\\\([&?!])/'quote107"\\1", $val);
                    if($key != $binds) {
    ...
    function &execute($stmt, $data = array()) {
        ...
        foreach($data as $key => $value) {
            if($types[$i] == DB_PARAM_MISC) {
                 ...
                 */
                $data[$key] = preg_replace("/^'(.*)'$/"quote107"\\1", $data[$key]);
        ...
        if(is_string($result)) {
            ...
            while(@OCIFetch($stmt)) {
                $res[$i] = array(
                    ...
                    'name' => $case_func(@OCIResult($stmt,
                    1)),
                    'type' => @OCIResult($stmt,
                    2),
                    'len' => @OCIResult($stmt,
                    3),
                    'flags' => (@OCIResult($stmt,
                    4) == 'N') ? 'not_null' : '',
                );
            ...
            if($result === $this->last_stmt) {
                ...
                for($i = 0; $i < $count; $i++) {
                    $res[$i] = array(
                        'table'      => '',
                        'name'       => $case_func(@OCIColumnName($result, $i+1)), 'type' => @OCIColumnType($result,
                        $i+1), 'len' => @OCIColumnSize($result, $i+1),
                        'flags'      => '',
                    ); if($mode&DB_TABLEINFO_ORDER) {
                        $res['order'][$res[$i]['name']] = $i;
                    }
