...
class Services_Yadis_PlainHTTPFetcher extends Services_Yadis_HTTPFetcher {
        ...
        while($redir && ($off > 0)) {
            ...
            if(!array_key_exists('port', $parts)) {
                ...
                elseif($parts['scheme'] == 'https') {
                    ...
                    trigger_error("fetcher post method doesn't support "." scheme '".$parts['scheme']."', no default port available", E_USER_WARNING);
                    return null;
                }
            }
            $host = $parts['host'];
            ...
            $headers = array(
                "GET ".$parts['path'].(array_key_exists('query',
                $parts) ? "?".$parts['query'] : "")." HTTP/1.0",
    ...
    function post($url, $body, $extra_headers = null) {
        ...
        if($sock === false) {
            trigger_error("Could not connect to ".$parts['host']." port ".$parts['port'], E_USER_WARNING);
            return null;
        }
        stream_set_timeout($sock, $this->timeout);
