...
class Services_Yadis_ParseHTML {
    ...
    function Services_Yadis_ParseHTML() {
        $this->_meta_find  = sprintf("/<meta\b(?!:)([^>]*)(?!<)>/%s", $this->_re_flags);
    ...
    function getMetaTags($html_string) {
        $stripped = preg_replace($this->_removed_re, "", $html_string);
        // Look for the closing body tag.
        $body_closer = sprintf($this->_close_tag_expr, 'body');
        $body_matches = array();
        preg_match($body_closer, $html_string, $body_matches, PREG_OFFSET_CAPTURE);
        if($body_matches) {
        ...
        $link_data = array();
        $link_matches = array();
        if(!preg_match_all($this->_meta_find, $head_matches[0], $link_matches)) {
            return array();
        }
