...
global $DEFAULT_PROXY, $XRI_AUTHORITIES;
...
function Services_Yadis_identifierScheme($identifier) {
    ...
    if(_startswith($identifier, 'xri://') || (in_array($identifier[0], $XRI_AUTHORITIES))) {
        return "XRI";
    }
    else {
        return "URI";
...
function Services_Yadis_iriToURI($iri) {
    if(1) {
        ...
        return preg_replace_callback($_escapeme_re, '_pct_escape_unicode', $iri);
    }
}
...
function Services_Yadis_rootAuthority($xri) {
    ...
    elseif(in_array($authority[0], $XRI_AUTHORITIES)) {
        // Other XRI reference.
        $root = $authority[0];
    }
    else {
