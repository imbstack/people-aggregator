...
class FortyThree {
    ...
    public function get_persons_entries($ft_uid, $offset = 0, $maxresults = 20) {
        $r       = $this->_request("43things", "get_persons_entries", "GET", array("id" => $ft_uid, "offset" => $offset, "max" => $maxresults,));
        $xp      = $r['xpath'];
        $entries = array();
        foreach($xp->query("/th:feed/th:entry") as $node) {
            $entries[] = $this->_convert_entry($xp, "th", $node);
        }
        return array("pagination" => $this->_extract_pagination($xp, "th"), "entries" => $entries,);
    }
    // wrapper for 43places.com's search_places function
    public function search_places($name) {
        $r      = $this->_request("43places", "search_places", "GET", array("q" => $name,));
        $xp     = $r['xpath'];
        $places = array();
        foreach($xp->query("/pl:feed/pl:place") as $node) {
            $places[] = array(
                "place_id" => $xp->evaluate("string(@place_id)",
                $node),
                "name" => $xp->evaluate("string(pl:name)",
                $node),
                "url" => $xp->evaluate("string(pl:link/@href)",
                $node),
                "num_registered_people" => $xp->evaluate("string(pl:num_registered_people)",
                $node),
                "id" => $xp->evaluate("string(pl:id)",
                $node),
            );
        }
        return array("pagination" => $this->_extract_pagination($xp, "pl"), "places" => $places,);
    }
    ...
    private function _extract_pagination($xp, $site, $path = NULL) {
        if(!$path) {
            $path = "/$site:feed/$site:pagination";
        }
        $pag = $xp->query($path)->item(0);
        return array("offset" => $xp->evaluate("string($site:offset)", $pag), "max" => $xp->evaluate("string($site:max)", $pag), "total" => $xp->evaluate("string($site:total)", $pag), "next_offset" => $xp->evaluate("string($site:next_offset)", $pag), "previous_offset" => $xp->evaluate("string($site:previous_offset)", $pag),);
    }

    private function _convert_entry($xp, $site, $node) {
        return array("title" => $xp->evaluate("string($site:title)", $node), "author_username" => $xp->evaluate("string($site:author/$site:username)", $node), "content" => $xp->evaluate("string($site:content[@mode='escaped'])", $node), "url" => $xp->evaluate("string($site:link[@rel='alternate']/@href)", $node),);
    }
}
?>