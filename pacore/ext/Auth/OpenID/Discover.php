...
class Auth_OpenID_ServiceEndpoint {
    ...
    function findDelegate($service) {
        ...
        $service->parser->registerNamespace('openid', 'http://openid.net/xmlns/1.0');
...
function filter_MatchesAnyOpenIDType(&$service) {
    ...
    foreach($uris as $uri) {
        if(in_array($uri, array(_OPENID_1_0_TYPE, _OPENID_1_1_TYPE, _OPENID_1_2_TYPE))) {
            return true;
        }
    }
    return false;
    ...
    foreach($endpoints as $service) {
        ...
        if($type_uris && $uris) {
            foreach($uris as $service_uri) {
                ...
                $openid_endpoint->parseService($uri, $service_uri, $type_uris, $service);
                $s[] = $openid_endpoint;
            }
        }
    }
...
function _Auth_OpenID_discoverServiceList($uri, &$fetcher) {
    list($url, $services, $resp) = Auth_OpenID_discoverWithYadis($uri, $fetcher);
    return $services;
}
