...
class Auth_OpenID_SQLStore extends Auth_OpenID_OpenIDStore {
     ...
     * value is 'oid_nonces'.
     */
    function Auth_OpenID_SQLStore($connection, $settings_table = null, $associations_table = null, $nonces_table = null) {
        global $__Auth_OpenID_PEAR_AVAILABLE;
    ...
    function tableExists($table_name) {
        return !$this->isError($this->connection->query("SELECT * FROM %s LIMIT 0", $table_name));
    }
    ...
    function reset() {
        ...
        $this->connection->query(sprintf("DELETE FROM %s", $this->settings_table_name));
    }
    ...
    function _create_auth($str) {
        return $this->connection->query($this->sql['create_auth'], array($str));
    }
    ...
    function _get_nonce($nonce) {
        $result = $this->connection->getRow($this->sql['get_nonce'], array($nonce));
        if($this->isError($result)) {
            return null;
        }
        else {
    ...
    function _remove_nonce($nonce) {
        $this->connection->query($this->sql['remove_nonce'], array($nonce));
    }
