...
global $_Auth_OpenID_Request_Modes, $_Auth_OpenID_OpenID_Prefix, $_Auth_OpenID_Encode_Kvform, $_Auth_OpenID_Encode_Url;

/**
 * @access private
 */
$_Auth_OpenID_Request_Modes = array(
    'checkid_setup',
    'checkid_immediate',
);
 ...
 * @access private
 */
$_Auth_OpenID_Encode_Kvform = array(
    'kfvorm',
);
...
class Auth_OpenID_ServerError {
    ...
    function hasReturnTo() {
        ...
        if($this->query) {
            return array_key_exists($_Auth_OpenID_OpenID_Prefix.'return_to', $this->query);
        }
        else {
            return false;
        }
    ...
    function encodeToURL() {
        global $_Auth_OpenID_OpenID_Prefix;
        $return_to = Auth_OpenID::arrayGet($this->query, $_Auth_OpenID_OpenID_Prefix.'return_to');
        if(!$return_to) {
    ...
    function whichEncoding() {
        global $_Auth_OpenID_Encode_Url, $_Auth_OpenID_Encode_Kvform, $_Auth_OpenID_Request_Modes;
        if($this->hasReturnTo()) {
...
class Auth_OpenID_CheckAuthRequest extends Auth_OpenID_Request {
    ...
    function Auth_OpenID_CheckAuthRequest($assoc_handle, $sig, $signed, $invalidate_handle = null) {
    ...
    function fromQuery($query) {
        ...
        foreach($required_keys as $k) {
            if(!array_key_exists($_Auth_OpenID_OpenID_Prefix.$k, $query)) {
                return new Auth_OpenID_ServerError($query, sprintf("%s request missing required parameter %s from \
        ...
        foreach($signed_list as $field) {
            if($field == 'mode') {
                ...
                if(array_key_exists($_Auth_OpenID_OpenID_Prefix.$field, $query)) {
                    ...
                    return new Auth_OpenID_ServerError($query, sprintf("Couldn't find signed field %r in query %s", $field, var_export($query, true)));
                }
            }
            $signed_pairs[] = array(
                $field,
        ...
        $result = new Auth_OpenID_CheckAuthRequest($assoc_handle, $sig, $signed_pairs);
        $result->invalidate_handle = Auth_OpenID::arrayGet($query, $_Auth_OpenID_OpenID_Prefix.'invalidate_handle');
        return $result;
    }

    function answer(&$signatory) {
        $is_valid = $signatory->verify($this->assoc_handle, $this->sig, $this->signed);
        ...
        if($this->invalidate_handle) {
            ...
            if(!$assoc) {
                $response->fields['invalidate_handle'] = $this->invalidate_handle;
            }
        }
        return $response;
    }
...
class Auth_OpenID_DiffieHellmanServerSession {
    ...
    function fromQuery($query) {
        ...
        if((($dh_modulus === null) && ($dh_gen !== null)) || (($dh_gen === null) && ($dh_modulus !== null))) {
            ...
            return new Auth_OpenID_ServerError('If non-default modulus or generator is '.'supplied, both must be supplied.  Missing '.$missing);
        }
        ...
        if($dh_modulus || $dh_gen) {
            ...
            if($lib->cmp($dh_modulus, 0) == 0 || $lib->cmp($dh_gen, 0) == 0) {
                return new Auth_OpenID_ServerError($query, "Failed to parse dh_mod or dh_gen");
            }
        ...
        $consumer_pubkey = Auth_OpenID::arrayGet($query, 'openid.dh_consumer_public');
        if($consumer_pubkey === null) {
            return new Auth_OpenID_ServerError('Public key for DH-SHA1 session '.'not found in query');
        }
        $consumer_pubkey = $lib->base64ToLong($consumer_pubkey);
        if($consumer_pubkey === false) {
            return new Auth_OpenID_ServerError($query, "dh_consumer_public is not base64");
        }
...
class Auth_OpenID_CheckIDRequest extends Auth_OpenID_Request {
    ...
    function Auth_OpenID_CheckIDRequest($identity, $return_to, $trust_root = null, $immediate = false, $assoc_handle = null) {
    ...
    function answer($allow, $server_url = null) {
        ...
        if($allow) {
            ...
            if(!$this->trustRootValid()) {
                return new Auth_OpenID_UntrustedReturnURL($this->return_to, $this->trust_root);
            }
        }
        else {
            ...
            if($this->immediate) {
                ...
                $setup_request                      = &new Auth_OpenID_CheckIDRequest($this->identity, $this->return_to, $this->trust_root, false, $this->assoc_handle);
    ...
    function getCancelURL() {
        ...
        return Auth_OpenID::appendArgs($this->return_to, array($_Auth_OpenID_OpenID_Prefix.'mode' => 'cancel'));
    }
}
...
class Auth_OpenID_ServerResponse {
    ...
    function whichEncoding() {
        global $_Auth_OpenID_Encode_Kvform, $_Auth_OpenID_Request_Modes, $_Auth_OpenID_Encode_Url;
...
class Auth_OpenID_Encoder {
    ...
    function encode(&$response) {
        global $_Auth_OpenID_Encode_Kvform, $_Auth_OpenID_Encode_Url;
        ...
        elseif($encode_as == $_Auth_OpenID_Encode_Url) {
...
function needsSigning($response) {
    return(in_array($response->request->mode, array('checkid_setup', 'checkid_immediate')) && $response->signed);
}
...
class Auth_OpenID_SigningEncoder extends Auth_OpenID_Encoder {
    ...
    function encode(&$response) {
        ...
        if(!is_a($response, 'Auth_OpenID_ServerError') && needsSigning($response)) {
            if(!$this->signatory) {
...
class Auth_OpenID_Decoder {
    ...
    function defaultDecoder($query) {
        ...
        return new Auth_OpenID_ServerError($query, sprintf("No decoder for mode %s", $mode));
    }
}
...
class Auth_OpenID_UntrustedReturnURL extends Auth_OpenID_ServerError {
    ...
    function toString() {
        ...
        return sprintf("return_to %s not under trust_root %s", $return_to, $trust_root);
    }
}
