...
class DOMIT_Utilities {
    ...
    */
    function DOMIT_Utilities() {
        die("DOMIT_Utilities Error: this is a static class that should never be instantiated.\n"quote54"Please use the following syntax to access methods of this class:\n"quote54'DOMIT_Utilities::methodName(parameters)');
    }
    //DOMIT_Utilities
    /**
    ...
    */
    function toNormalizedString(&$node, $subEntities = false, $definedEntities) {
        ...
        if($node->nodeType == DOMIT_DOCUMENT_NODE) {
            ...
            for($i = 0; $i < $total; $i++) {
                $response .= DOMIT_Utilities::getNormalizedString($node->childNodes[$i], $node_level, $subEntities, $definedEntities);
            }
            return $response;
        }
        else {
            return($response.DOMIT_Utilities::getNormalizedString($node, $node_level, $subEntities, $definedEntities));
        }
    }
    //toNormalizedString
    /**
    ...
    function getNormalizedString(&$node, $node_level, $subEntities = false, $definedEntities) {
        ...
        switch($node->nodeType) {
            case DOMIT_ELEMENT_NODE:
                $response .= DOMIT_Utilities::getNormalizedElementString($node, $response, $node_level, $subEntities, $definedEntities);
                break;
            case DOMIT_TEXT_NODE:
                if($node->nextSibling == null) {
                    $node_level--;
                }
                $response .= ($subEntities ? DOMIT_Utilities::convertEntities($node->nodeValue, $definedEntities) : $node->nodeValue);
                break;
