...
class php_http_client_generic extends php_http_request {
    ...
    */
    function &send($message) {
        ...
        if($conn->isOpen()) {
            //build header info
            $request  = $this->requestMethod.' '.$this->requestPath.' '.$this->protocol.'/'.$this->protocolVersion.CRLF;
            ...
            while(!feof($connResource)) {
                ...
                switch($readState) {
                    ...
                    case HTTP_READ_STATE_HEADERS:
                        if(trim($data) == '') {
                            //end of headers is signalled by a blank line
            ...
            $headerString = is_object($this->response->headers) ? $this->response->headers->toString() : $this->response->headers;
            ...
            HTTPExceptions::raiseException(HTTP_SOCKET_CONNECTION_ERR, ('HTTP Transport Error - Unable to establish connection to host '.$conn->host));
            return $this->response;
        }
    }
    //send
    ...
    */
    function normalizeResponseIfChunked() {
        if(($this->protocolVersion = '1.1') && (!$this->response->isResponseChunkDecoded)) {
            if($this->responseHeadersAsObject) {
                if($this->response->headers->headerExists('Transfer-Encoding') && ($this->response->headers->getHeader('Transfer-Encoding') == 'chunked')) {
                    $this->response->message = $this->decodeChunkedData($this->response->getResponse());
                    $this->response->isResponseChunkDecoded = true;
                }
            }
            else {
                if((strpos($this->response->headers, 'Transfer-Encoding') !== false) && (strpos($this->response->headers, 'chunked') !== false)) {
                    $this->response->message = $this->decodeChunkedData($this->response->getResponse());
                    $this->response->isResponseChunkDecoded = true;
                }
            }
