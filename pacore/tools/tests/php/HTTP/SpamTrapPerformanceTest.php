...
class SpamTrapPerformanceTest extends PA_TestCase {
    ...
    function testSpamTrapPerformance() {
        ...
        foreach($content as $url => $foo) {
            ...
            $c = new HTTP_Client;
            $c->setMaxRedirects(0);
            $ret = $c->post($url, //	    list(,,$t_taken, $c) = $this->get_and_parse($url, NULL, "POST",
            array("addcomment" => "Submit Comment", "name" => "automatic test robot", "email" => "peepagg-autotest@myelin.co.nz", "homepage" => "http://peopleaggregator.net/", "cid" => $cid, "ccid" => "",));
            var_dump($c);
