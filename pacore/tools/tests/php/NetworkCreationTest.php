...
class NetworkCreationTest extends PHPUnit_Framework_TestCase {
    ...
    function testNetworkCreation() {
        ...
        // make a new network
        $net = new Network();
        $net->set_params(array('user_id' => $user->user_id, 'name' => "auto-test network ($name)", 'address' => $name, 'tagline' => "not much of a tagline", 'category_id' => 8, // computers & internet
        'type' => 0, // public=0, private=2
        'description' => "This network has been created automatically by a PHPUnit test.  If the test succeeds, it will be deleted, too!", 'extra' => serialize($network_basic_controls), 'created' => time(), 'changed' => time(),));
        $net->save();
