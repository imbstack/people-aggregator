...
function download($url, $path) {
    ...
    while(1) {
        ...
        if(preg_match("|^HTTP/[^ ]+ (\d+) (.*)$|", $line, $m)) {
            list(, $code, $msg) = $m;
            if($code != 200) {
                fail("received bad status from server: $code $msg");
            }
        }
    ...
    if($content_length < 0) {
        fail("missing Content-Length header in response");
    }
    ...
    while($bytes_left > 0) {
        set_time_limit(60);
        $get_len = 128*1024;
        if($get_len > $bytes_left) {
            $get_len = $bytes_left;
        }
        $data = fread($s, $get_len+1);
        if(!$data) {
            fail("an error occurred reading data from the socket");
        }
...
if(!$zip_fn) {
    fail("error: can't find the filename of the latest zipped release on update.peopleaggregator.org");
}
