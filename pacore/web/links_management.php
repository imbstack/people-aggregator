...
if(!empty($_POST['btn_apply_name'])) {
    if($_POST['form_action'] == "update") {
        ...
        elseif(!Validation::validate_alpha_numeric($_POST['category_name'], 1)) {
...
elseif(!empty($_POST['btn_save_link'])) {
    if(empty($_POST['link_categories'])) {
        $error_array[] = "Please select a category";
    }
...
elseif(!empty($_POST['form_action']) && $_POST['form_action'] == "delete_links" && count($_POST['link_id']) > 0) {
...
elseif(!empty($_POST['form_action']) && $_POST['form_action'] == "delete_category" && !empty($_POST['link_categories'])) {
    $param_array = array(
        'user_id'     => $_SESSION['user']['id'],
...
$page = new PageRenderer("setup_module", PAGE_LINKS_MANAGEMENT, sprintf(__("%s - My Links - %s"), $login_user->get_name(), PA::$network_info->name), "container_three_column.tpl", "header.tpl", PUB, HOMEPAGE, PA::$network_info);
