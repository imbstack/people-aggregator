    ...
    if(is_array($_POST['route_targets_group'])) {
        ...
        elseif(in_array(-1, $_POST['route_targets_group'])) {
            ...
            foreach($group_array as $gid) {
                ...
                elseif($extra['network_content_moderation'] == NET_YES && $is_edit == 0 && PA::$network_info->owner_id != $user->user_id) {
...
if(@$_POST['route_to_pa_home'] == 1) {
    $display_on = 0;
    $route_to_pa = TRUE;
    // The content will be displayed on the homepage also.
}
...
function route_to_outputthis($title, $body) {
    ...
    if(count($external_targets)) {
        ...
        elseif(in_array(ROUTE_TO_ALL, $external_targets)) {
        ...
        for($i = 0; $i < count($external_targets); $i++) {
            $blog_post = array(
                ...
                $return = OutputThis::send($blog_post, array($external_targets[$i]), $outputthis_username, $outputthis_password); if($return == 1) {
