...
// because this page must be visible even if you are not logged in and on a private network!
$use_theme = 'Beta';
...
if(isset(PA::$login_uid) && isset($_SESSION['login_source']) && $_SESSION['login_source'] == 'password' && empty($_GET['action']) && empty($_GET['enable'])// this is if we want to enable a new ID for this user && empty($_GET['auth'])// second step in a login form && empty($_GET['openid_mode'])// this is if we come in via OpenID and are already logged in && empty($_GET['GInvID'])) {
