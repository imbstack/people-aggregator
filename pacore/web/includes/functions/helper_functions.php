...
function pa_log_script_execution_time($at_start = FALSE) {
    ...
    if($at_start) {
        $msg = sprintf("%d\tstart\t%s\t%s\t%s\t%s\t%s\thttp://%s\t%s\t%s\t%s\t%s\n", posix_getpid(), PA::datetime(time(), 'long', 'short'), //date("Y-m-d H:i:s"),
        $_SERVER['REMOTE_ADDR'], @$_SERVER['HTTP_X_FORWARDED_FOR'], $status_map[connection_status()], $_SERVER['REQUEST_METHOD'], $_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'], $_SERVER['HTTP_USER_AGENT'], implode("&", $post), @$_SESSION['user']['id'], @$_SESSION['user']['name']);
    }
    else {
...
function _n($fmt, $n) {
    ...
    foreach(preg_split("/\n/", $fmt) as $line) {
        ...
        if(!preg_match("/^(\d*)(?:\-(\d+))?;(.*)$/", $line, $m)) {
            throw new PAException(GENERAL_SOME_ERROR, "Invalid line format for _n(): '$line'");
        }
    ...
    if(!$translation) {
        throw new PAException(GENERAL_SOME_ERROR, "Couldn't find translation for format '$fmt' and number=$n");
    }
