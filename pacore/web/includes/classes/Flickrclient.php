...
class Flickrclient {
    ...
    public function __construct() {
        ...
        // set up api wrapper
        $this->api = &new Flickr_API(array('api_key' => $this->api_key, 'api_secret' => $this->api_secret,));
    ...
    public function auth_getToken($frob) {
        ...
        return array("token"  => $this->user_token, "perms" => $xp->query("/rsp/auth/perms")->item(0)->textContent, "nsid" => $xp->query("/rsp/auth/user/@nsid")->item(0)->value, "username" => $xp->query("/rsp/auth/user/@username")->item(0)->value, "fullname" => $xp->query("/rsp/auth/user/@fullname")->item(0)->value,);
    }
    // given an user's nsid get their (public) infp
    public function people_getInfo($nsid) {
        $r = $this->_api_callmethod("flickr.people.getInfo", array("user_id" => $nsid));
        return $r;
    }
    // get a user's contact list (requires read authentication)
    function contacts_getList() {
        if(!$this->user_token) {
            throw new PAException(OPERATION_NOT_PERMITTED, "You must authenticate against flickr before calling contacts_getList()");
        }
        ...
        foreach($xp->query("/rsp/contacts/contact") as $node) {
            $contacts[] = array(
                "nsid" => $xp->query("@nsid",
                $node)->item(0)->value,
                "username" => $xp->query("@username",
                $node)->item(0)->value,
                "iconserver" => $xp->query("@iconserver",
                $node)->item(0)->value,
                "realname" => $xp->query("@realname",
                $node)->item(0)->value,
                "friend" => $xp->query("@friend",
                $node)->item(0)->value,
                "family" => $xp->query("@family",
                $node)->item(0)->value,
                "ignored" => $xp->query("@ignored",
                $node)->item(0)->value,
            );
        }
        return $contacts;
