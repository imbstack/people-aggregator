...
class FileUploader {
    ...
    */
    private function set_file_name($file) {
        $this->raw_file_name = $file;
        // 'insecure' filename, to pass to Storage
        ...
        if(!empty($check_type)) {
            ...
            if($check_type == 'audio' || $check_type == 'video') {
                if(CHECK_MIME_TYPE == 1) {
                    ...
                    elseif(strstr($mime_type, 'media')) {
                    }
                    elseif(strstr($mime_type, 'octet-stream')) {
            ...
            if($check_type == 'doc') {
                if(CHECK_MIME_TYPE == 1) {
                    $mime_type = exec('file -bi '.$this->tmp_name);
                    if(strstr($mime_type, 'msword') || strstr($mime_type, 'pdf') || strstr($mime_type, 'text/plain')) {
                        //ToDo: Temporary fix. Need to correct it
                    }
                    else {
