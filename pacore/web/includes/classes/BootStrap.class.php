...
class BootStrap {
    ...
    private function collectSystemData() {
        ...
        $this->current_scheme = PA_CURRENT_SCHEME;(isset($_SERVER['HTTP_X_FORWARDED_HOST'])) ? define('PA_HTTP_HOST', $_SERVER['HTTP_X_FORWARDED_HOST']) : define('PA_HTTP_HOST', $_SERVER['HTTP_HOST']);
        $this->http_host      = PA_HTTP_HOST;(isset($_SERVER['HTTP_X_FORWARDED_SERVER'])) ? define('PA_SERVER_NAME', $_SERVER['HTTP_X_FORWARDED_SERVER']) : define('PA_SERVER_NAME', $_SERVER['SERVER_NAME']);
        ...
        define('PA_BASE_URL', PA_CURRENT_SCHEME.'://quote22'.PA_SERVER_NAME);
    ...
    public function autoLoadFiles($config_scheme) {
        ...
        foreach($config_scheme as $file_info) {
            try {
                ...
                $function_variable_names = array(
                    "function_variable_names" => 0,
                    "before_eval_vars"        => 0,
                    "created"                 => 0,
                );
    ...
    public function loadConfigFile($fname) {
        ...
        elseif(file_exists(PA::$core_dir."{$fname}.distr")) {
    ...
    **/
    ...
    public function detectDBSettings() {
        ...
        $peepagg_dsn = "mysql://".PA::$config->db_user.":".PA::$config->db_password."@".PA::$config->db_host."/".PA::$config->db_name;
    ...
    private function parseConfigFile($name, $values) {
        ...
        foreach($values as $_name => $_value) {
            ...
            if((($type <> 'array') && ($type <> 'multi_array')) && is_array($_value)) {
                ...
                elseif(($category == 'dynamic_var') || ($category == 'static_var') || ($category == 'item')) {
                    $var_value = null;
                    switch($type) {
                        case 'bool':
                        case 'int':
                            $exp = "\$var_value = (int)$_value; return true;";
                            break;
                        case 'array':
                            $exp = "\$var_value = \$_value; return true;";
                            break;
    ...
    **/
    private function afterParse() {
        ...
        if(PA::$config->use_storage) {
            define("NEW_STORAGE", TRUE);
        }
    ...
    public function getLanguagesList() {
        ...
        $language_dirs = array(
            PA::$core_dir.DIRECTORY_SEPARATOR.PA_LANGUAGES_DIR,
        ...
        foreach($language_dirs as $lang_path) {
            foreach(new RecursiveDirectoryIterator($lang_path, RecursiveDirectoryIterator::KEY_AS_PATHNAME) as $_path => $info) {
                ...
                if($info->isDir() && $_entry != '.svn') {
                    if(file_exists($_path.DIRECTORY_SEPARATOR.'strings.php')) {
                        // NOTE: language strings from paproject
                        $languages_list[$_entry] = $_path.DIRECTORY_SEPARATOR.'strings.php';
                        // will override strings from the core
                    }
                }
            }
        }
    ...
    public function check_ip_ban() {
        ...
        if(file_exists($ban_path)) {
            ...
            echo "ip parse failed!";
            // fake error message so if someone legitimate gets banned, we know what happened.
            exit;
        }
    }
