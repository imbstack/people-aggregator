...
class pAhCardXfn extends hCardXFN {
    ...
    private $ctype_depended = array(
        'org',
        'organization-name',
        'locality',
        'title',
    );
    // depended of the company type
    ...
    public function hCardToPaProfile($indx) {
        ...
        // remove "type" attrib from email record
        $card_data = $this->outData[$indx]['value'];
        $this->company_type = HCARD_CURRENT_COMPANY;
    ...
    public function getRelationsAsArray($rselected) {
        ...
        foreach($rselected as $k => $v) {
            ...
            $network_uid = (isset($relation['value']['xfnfriendly'])) ? $relation['value']['xfnfriendly']// Tantek stuff :) : $relation['value']['fn'];
            ...
            $this->relations_records['relation'][$cnt]['thumbnail_url'] = (isset($relation['value']['photo'])) ? $relation['value']['photo'] : '';
            $this->relations_records['relation'][$cnt]['profile_url'] = (isset($relation['value']['url'])) ? $relation['value']['url'] : '';
            ++$cnt;
        }
            ...
            if(is_array($v)) {
                ...
                if(count($res) > 0) {
                    if(in_array($k, $this->ctype_depended)) {
                        $path_arr = explode('/', $res[$this->company_type]);
                        // depended of the company type
                        $section = $path_arr[0];
                        $name = $path_arr[1];
                        $value = trim($v, " ;");
                    ...
                    elseif(($k == 'given-name') || ($k == 'family-name')) {
                        // multiple defined (in 'core' and 'basics')
