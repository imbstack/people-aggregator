...
define('DEFAULT_MSN_LIVE_PROTOCOL',  'tls'); // NOTE: Microsoft plans to discontinue the use of the SSLv2 (Secure Socket Layer)
...
class SimpleMSLiveClient extends ServiceProtocolHandler {
 ...
 public function getLastError() {
  ...
  private function getAuthToken($user_id, $password) {
    ...
    $xml_tpl = (file_exists(PA::$project_dir.MSN_LIVE_XML_FILE)) ? PA::$project_dir.MSN_LIVE_XML_FILE
                                                                 : PA::$core_dir.MSN_LIVE_XML_FILE;
    ...
    if(!empty($response)) {
      ...
      if(preg_match($regexp, $response, $matches)) {
        ...
        if($err_entries->length > 0) {
           ...
           $this->status_code = (isset($psf_error_code)) ? $psf_error_code : 500; // '500' -> Unknown error
