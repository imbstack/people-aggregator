...
class Navigation {
    ...
    **/
    function make_links() {
        ...
        elseif(!$this->is_anonymous && $is_member && !$owner && $this->network_info->type != MOTHER_NETWORK_TYPE) {
        ...
        elseif((Network::is_admin($this->network_info->network_id, (int) $user_id)) || // fix by Z.Hron - on David's suggestion: User with any administration task should be able to access to Configure Network.(($user_id > 0) and Roles::check_administration_permissions((int) $user_id))) {
            $level_1['configure_network'] = array(
        ...
        $user_children = array();
        $user_children = $user_children+array('user_private' => array(
            'caption' => __('My Page'),
            'url' => $this->base_url.PA_ROUTE_USER_PRIVATE,
        ),);
        $user_children = $user_children+array('user_widgets' => array(
            'caption' => __('My Widgets'),
            'url' => $this->base_url.'/'.FILE_WIDGET,
        ),);
        $user_children = $user_children+array('messages' => array(
            'caption' => __('My Messages'),
            'url' => $this->base_url.PA_ROUTE_MYMESSAGE,
        ),);
        $user_children = $user_children+array('my_gallery' => array(
            'caption' => __('My Gallery'),
            'url' => $this->base_url.PA_ROUTE_MEDIA_GALLEY_IMAGES."/uid=$uid",
        ),);
        $user_children = $user_children+array('my_events' => array(
            'caption' => __('My Events'),
            'url' => $this->base_url.'/'.FILE_USER_CALENDAR,
        ), 'my_friends' => array(
            'caption' => __('My Friends'),
            'url' => $this->base_url."/view_all_members.php?view_type=in_relations&amp;uid=$uid",
        ),);
        $user_children = $user_children+array('my_forum' => array(
            'caption' => __('My Forum'),
            'url' => $this->base_url.PA_ROUTE_FORUMS."/network_id=".$this->network_info->network_id.'&user_id='.$uid,
        ),);
        $user_children = $user_children+array('my_points' => array(
            'caption' => __('My Points'),
        ...
        if(!empty(PA::$config->simple['use_families'])) {
            ...
            if(count($userfamilyRelations) == 1) {
                $user_children = $user_children+array('my_family' => array(
                    'caption' => __('My Family'),
                ...
                $html .= "</ul>";
                $user_children = $user_children+array('my_family' => array(
                    'caption' => __('My Families'),
                    'html' => $html,
                ));
            }
        }
        // end of !empty(PA::$config->simple['use_families'])
        $user_children = $user_children+array('settings' => array(
            'caption' => __('Edit My Account'),
            'url' => $this->base_url.PA_ROUTE_EDIT_PROFILE,
        ),);
        $user_children = $user_children+array('customize_ui' => array(
            'caption' => __('Themes'),
        ...
        $people_children = array(
            'find_people' => array(
                'caption' => sprintf(__('Find %s'),
                __(PA::$people_noun)),
                'url' => $this->base_url.PA_ROUTE_PEOPLES_PAGE,
            ),
            'my_friends' => array(
            ...
            'friends_gallery' => array(
                'caption' => __('Friends gallery'),
        ...
        $groups_general = array(
            'find_groups' => array(
                'caption' => sprintf(__('Find %s'),
                __(PA::$group_noun_plural)),
                'url' => $this->base_url.PA_ROUTE_GROUPS,
            ),
            'create_group' => array(
                'caption' => __('Create'),
        ...
        elseif(!empty($group_id) && !Group::is_admin($group_id, $user_id)) {
            unset($group_specific['edit_group']);
            unset($group_specific['delete_group']);
            unset($group_specific['moderate_group']);
            ...
            elseif(Group::member_exists($group_id, $user_id) == FALSE) {
                unset($group_specific['unjoin']);
            }
            unset($group_specific['group_customize_ui']);
        }
        ...
        $manage_network = array(
            ...
            'manage_content' => array(
                'caption' => __('Manage Contents'),
                'url' => $this->base_url.'/'.FILE_NETWORK_MANAGE_CONTENT,
            ),
            'manage_links' => array(
                'caption' => __('Manage Links'),
        ...
        $network_stats = array(
            ...
            'customize_ui' => array(
                'caption' => __('Customize UI'),
                'url' => $this->base_url.'/'.FILE_NETWORK_CUSTOMIZE_UI_PAGE,
            ),
        );
        $network_module_selector = array(
            'home_page_id' => array(
                'caption' => __('Home Page'),
                'url' => $this->base_url.'/'.FILE_MODULE_SELECTOR.'?page_id=home_page_id',
            ),
            'user_default_page_id' => array(
                'caption' => __('User Default Page'),
                'url' => $this->base_url.'/'.FILE_MODULE_SELECTOR.'?page_id=user_default_page_id',
            ),
            'group_directory_page_id' => array(
                'caption' => __('Group Directory Page'),
                'url' => $this->base_url.'/'.FILE_MODULE_SELECTOR.'?page_id=group_directory_page_id',
            ),
            'network_directory_page_id' => array(
                'caption' => __('Network Directory Page'),
                'url' => $this->base_url.'/'.FILE_MODULE_SELECTOR.'?page_id=network_directory_page_id',
            ),
        );
        if($this->network_info->type != MOTHER_NETWORK_TYPE) {
    ...
    **/
    public function get_links($optional = NULL) {
        ...
        elseif((FILE_FORUM_MESSAGES == $this->current_page || FILE_CONTENT == $this->current_page) && !empty($_REQUEST['ccid']) && $_REQUEST['ccid'] > 0) {
            $this->set_group_id($_REQUEST['ccid']);
            $is_group_content = TRUE;
        }
        elseif(PAGE_PERMALINK == $this->current_page && !empty($_GET['cid'])) {
            try {
        ...
        elseif(!($this->network_info) && ($_SESSION['user']['id'] == SUPERUSER)) {
            $level_3 = $this->get_level_3('network');
        }
        $level_3 = NULL;
        switch($this->current_page) {
            ...
            case PAGE_SEARCH:
                ...
                elseif(!empty($_GET['btn_searchContent'])) {
            ...
            case PAGE_USER_PRIVATE:
                ...
                if(PA::$page_uid) {
                    ...
                    $relation_already_exists_links = array(
                        'send_message' => array(
                            'caption' => __('Send a message'),
                            'url' => $this->base_url.PA_ROUTE_ADDMESSAGE.'/uid='.PA::$page_uid,
                        ),
                        'change_relationship' => array(
                            'caption' => __('Change Relation'),
                            'url' => $this->base_url.PA_ROUTE_EDIT_RELATIONS.'/uid='.PA::$page_uid.'&amp;do=change&amp;action=EditRelation',
                        ),
                        'delete_relationship' => array(
                            'caption' => sprintf(__('Delete as %s'),
                            __($def_relations_term)),
                    ...
                    $relation_does_not_exists_links = array(
                        'send_message' => array(
                            'caption' => __('Send a message'),
                            'url' => $this->base_url.PA_ROUTE_ADDMESSAGE.'/uid='.PA::$page_uid,
                        ),
                        'make_connection' => array(
                            'caption' => sprintf(__('Add as %s'),
                            __($def_relations_term)),
            ...
            case FILE_VIEW_ALL_MEMBERS:
                if(@$_GET['gid']) {
                    ...
                    if(PA::$page_uid == PA::$login_uid) {
                        ...
                        elseif((!empty($_GET['view_type'])) && ($_GET['view_type'] == 'in_relations')) {
            ...
            case PAGE_MEDIA_GALLERY:
                if(PA::$login_uid) {
                    ...
                    elseif(isset($_GET['view']) && 'friends' == $_GET['view']) {
                        //user is viewing his friends gallery
                        $level_2['highlight'] = 'people';
                        $level_3              = $this->get_level_3('people');
                        $level_3['highlight'] = 'friends_gallery';
                    }
                    elseif((PA::$page_uid != PA::$login_uid) && (PA::$page_uid != '')) {
        ...
        */
        ...
        $menu = array(
            ...
            'level_2'                => $level_2,
            'level_3'                => $level_3,
            'left_user_public_links' => $left_user_public_links,
        );
