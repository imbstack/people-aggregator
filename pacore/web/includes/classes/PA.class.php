...
class PA {
    ...
    public static $path;
    // absolute path to the root of the site files (web/..); same as $path_prefix
    public static $url;
    // absolute url of the web subdirectory;
    public static $local_url;
    // relative url of the web subdirectory; same as BASE_URL_REL (relative to root of url)
    public static $upload_path;
    // absolute path to web/files directory; same as $uploaddir - DEPRECATED; use Storage functions instead!
    public static $theme_url;
    // absolute url to web/Themes/Default directory; same as $current_theme_path
    public static $theme_rel;
    // relative url to web/Themes/Default directory; same as $current_theme_rel_path (relative to web directory)
    public static $theme_path;
    // path to web/Themes/XYZ directory
    public static $blockmodule_path;
    // absolute path to web/BlockModules directory
    public static $domain_suffix = FALSE;
    // copy of $domain_suffix from local_config.php
    public static $network_capable = FALSE;
    // TRUE if we are capable of running multiple networks, i.e. wildcard DNS is configured
    // current network
    public static $network_info = NULL;
    // Network object for current network
    // logged-in user
    public static $login_uid = NULL;
    // uid of logged-in users
    public static $login_user = NULL;
    // User object for logged-in user
    // user specified on the url
    public static $page_uid = NULL;
    // uid specified in 'uid' (user_id) or 'user' (login_name) on the query string (if valid)
    public static $page_user = NULL;
    // User object corresponding to $page_uid
    ...
    public static function date($date, $format = 'long') {
        if(!empty(self::$language) && !empty(self::$culture_data[self::$language]['date_time'])) {
            ...
            $date_format = self::$culture_data['default']['date_time'][$format]['date'];
            // en_US is default culture
        }
    ...
    public static function time($time, $format = 'long') {
        if(!empty(self::$language) && !empty(self::$culture_data[self::$language]['date_time'])) {
            ...
            $date_format = self::$culture_data['default']['date_time'][$format]['time'];
            // en_US is default culture
        }
    ...
    public static function datetime($date, $date_format = 'long', $time_format = 'long') {
        if(!empty(self::$language) && !empty(self::$culture_data[self::$language]['date_time'])) {
            ...
            $date_format = self::$culture_data['default']['date_time'][$date_format]['date'];
            // en_US is default culture
    ...
    public static function getCountryList($language = null) {
        ...
        elseif(!empty(self::$language) && !empty(self::$culture_data[self::$language]['countries'])) {
            $country_list = self::$culture_data[self::$language]['countries'];
        }
        else {
            $country_list = self::$culture_data['default']['countries'];
            // en_US is default culture
        }
        return $country_list;
    }
    ...
    public static function getStatesList($language = null) {
        ...
        elseif(!empty(self::$language) && !empty(self::$culture_data[self::$language]['states'])) {
            $states_list = self::$culture_data[self::$language]['states'];
        }
        else {
            $states_list = self::$culture_data['default']['states'];
            // en_US is default culture
        }
        return $states_list;
    }
    ...
    public static function getMonthsList($language = null) {
        ...
        elseif(!empty(self::$language) && !empty(self::$culture_data[self::$language]['months'])) {
            $months_list = self::$culture_data[self::$language]['months'];
        }
        else {
            $months_list = self::$culture_data['default']['months'];
            // en_US is default culture
        }
        return $months_list;
    }
    ...
    public static function getDaysList($language = null) {
        ...
        elseif(!empty(self::$language) && !empty(self::$culture_data[self::$language]['days'])) {
            $days_list = self::$culture_data[self::$language]['days'];
        }
        else {
            $days_list = self::$culture_data['default']['days'];
            // en_US is default culture
        }
        return $days_list;
    }
    ...
    public static function resolveRelativePath($path) {
        ...
        elseif(file_exists(self::$core_dir.DIRECTORY_SEPARATOR.$path)) {
