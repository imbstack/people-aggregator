...
class DynamicPage extends ConfigurablePage {
    ...
    public function __destruct() {
        if($this->save_page) {
            // page data will be stored on exit
            parent::__destruct();
        }
    }
    ...
    private function checkXmlConfigStructure($new_settings) {
      ...
      ";
        ...
        if(isset($new_settings['page_name'])

        /* && ($new_settings['page_name'] != null) */) {
        ...
        elseif(!$this->hasConfigSection('page_name', '//page')) {
        ...
        elseif(!$this->hasConfigSection('is_configurable', '//page')) {
        ...
        foreach($layouts as $layout) {
            if(isset($new_settings[$layout])

            /* && ($new_settings[$layout] != null)*/) {
                $this->removeConfigSection($layout, '//page/data');
                // clear modules data
            ...
            elseif(!$this->hasConfigSection($layout, '//page/data')) {
                $this->addConfigSection($layout, '//page/data');
            }
        }
        if(isset($new_settings['javascripts'])

        /* && ($new_settings['javascripts'] != null) */) {
            $this->removeConfigSection('javascripts', '//page/data');
            // clear javascripts data
        ...
        elseif(!$this->hasConfigSection('javascripts', '//page/data')) {
            $this->addConfigSection('javascripts', '//page/data');
        }
        if(isset($new_settings['page_css'])

        /* && ($new_settings['page_css'] != null) */) {
            $this->removeConfigData('page_css', '//page/data');
            // clear CSS data
        ...
        elseif(!$this->hasConfigSection('page_css', '//page/data')) {
            $this->addConfigSection('page_css', '//page/data');
        }
        $defaults = array(
        ...
        foreach($defaults as $param_name => $defaul_value) {
            $value = '';
            if(isset($new_settings[$param_name])

            /* && ($new_settings[$param_name] != null) */) {
            ...
            elseif(!$this->getConfigData($param_name, '//page/data')) {
                $value = $defaul_value;
                if(($param_name == 'boot_code') || ($param_name == 'navigation_code') || ($param_name == 'body_attributes')) {
                    $this->addConfigData($param_name, $value, '//page/data', true);
                }
    ...
    public function getJavascripts() {
        $scripts = array();
        if($_scripts = $this->getConfigData('item', 'quote56//page/data/javascripts')) {
            $scripts = $_scripts;
        }
        return $scripts;
    }
    ...
    public function addJavascripts($scriptname) {
        $this->addConfigData('item', $scriptname, 'quote56//page/data/javascripts');
        //     $this->initialize();
    }

    public function removeJavascripts($scriptname) {
        $this->removeConfigData('item', 'quote56//page/data/javascripts', $scriptname);
        //     $this->initialize();
    }

    public function getPageCSS() {
        $css_fnames = array();
        if($_css_fnames = $this->getConfigData('item', 'quote56//page/data/page_css')) {
    ...
    public function addPageCSS($fname) {
        $this->addConfigData('item', $fname, 'quote56//page/data/page_css');
        //     $this->initialize();
    }

    public function removePageCSS($fname) {
        $this->removeConfigData('item', 'quote56//page/data/page_css', $fname);
        //     $this->initialize();
    }
