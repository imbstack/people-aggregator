...
function uihelper_upload_gallery($uid, $_POST, $_FILES, $type, $k = 0) {
    ...
    elseif(empty($_FILES[$file_name_dynamic]['name']) && !empty($_POST['userfile'.$type.'_url_'.$k])) {
...
function uihelper_upload_gallery_for_group($uid, $_POST, $_FILES, $type, $k = 0) {
    ...
    $array_of_error_message = array(
        ...
        @$error_file,
        @$uploaded,
        'collection_id' => $new_img->parent_collection_id,
        'content_id' => $new_img->content_id,
    );
    return $array_of_error_message;
}
...
function manage_user_desktop_image($image, $desktop_image_action, $options = NULL) {
    ...
    $img_desktop_info = uihelper_resize_img($image, 1024, 150, PA::$theme_rel."/images/default_desktop_image.jpg"quote399'alt="Desktop image."', $opts);
...
function uihelper_explain_desktop_image_action($desktop_image_action) {
    ...
    switch($desktop_image_action) {
        case DESKTOP_IMAGE_ACTION_TILE:
            $opts = RESIZE_FIT;
            // shrink or expand the image to fit inside 1016x191, then we'll repeat it from there
            $repeat = 'repeat';
            break;
        case DESKTOP_IMAGE_ACTION_CROP:
            $opts = RESIZE_CROP;
            // shrink or expand the image to > 1016x191, then crop the center out
            $repeat = 'no-repeat';
            break;
        case DESKTOP_IMAGE_ACTION_STRETCH:
            $opts = RESIZE_STRETCH;
            // ignore aspect ratio and stretch the image to 1016x191
            $repeat = 'no-repeat';
            break;
...
function get_network_css() {
    ...
    if(!empty(PA::$network_info)) {
        ...
        if(empty($extra['network_skin'])) {
            $extra['network_skin'] = 'defaults';
            //default network skins
        }
        $skin_info                              = skin_details($extra['network_skin']);
        $css                                    = @$skin_info['networkCssFile'];
        $result[$extra['network_skin'].'_skin'] = PA::$theme_url.'/skins/'.$extra['network_skin'].'/'.$css;
    }
    return $result;
}
...
function get_network_image() {
    ...
    elseif(file_exists(PA::$core_dir."/web/files/".$extra['basic']['header_image']['name'])) {
    ...
    if(empty($extra['basic']['header_image']['name']) || ($header_image_file == null)) {
        ...
        if($image_attrib = getimagesize($header_image_file)) {
            ...
            $header_image_temp      = uihelper_resize_img($extra['basic']['header_image']['name'], 1024, 191, PA::$theme_rel."/images/header_image.jpg"quote399'alt="Desktop image."', $opts);
...
function make_css($data) {
    if($data['profile_type'] == 'ui') {
        ...
        elseif(isset($data['restore_default'])) {
            foreach($data as $k => $v) {
                unset($profile[$k]);
...
function get_user_theme($uid = null) {
    $current_skin = array();
    $current_skin['css_files'] = array(
        PA::$theme_url.'/network.css',
    );
    if(!empty($uid)) {
        $user          = new User();
        $user->user_id = $uid;
...
function get_skins($skin_type = 'network') {
    ...
    foreach($skins_directories as $skins_dir) {
        ...
        while(false !== ($file = readdir($handle))) {
            ...
            if(file_exists("$skins_dir/$file/disabled")) {
                continue;
            }
                ...
                case 'network':
                    if(!empty($skin_info['networkCssFile'])) {
                        $skins[] = array(
                            'name'        => $file,
                            'css'         => $skin_info['networkCssFile'],
                            'preview'     => $skin_info['previewImage'],
                            'caption'     => $skin_info['name'],
                            'headerImage' => $skin_info['headerImageOk'],
                        );
                    }
                    break;
                case 'user':
...
function skin_details($skin_name) {
    ...
    elseif(file_exists(PA::$core_dir.$config_file)) {
    ...
    if($config_file) {
        ...
        foreach($config_params as $param) {
            ...
            if(isset($node_obj->item(0)->nodeValue) && ($node_obj->item(0)->nodeValue != 'none')) {
...
function get_group_theme($gid) {
    ...
    $current_skin['css_files'][]          = PA::$theme_url.'/skins/'.$skin_var['theme'].'/'.$skin_info['groupCssFile'];
...
function authenticate_invitation_token($token) {
    ...
    if(empty($msg)) {
        // if token doesn't contain invalid formats
