    ...
    switch($_REQUEST['config_action']) {
            ...
            if(!empty($_FILES['local_file']['name']) && is_uploaded_file($_FILES['local_file']['tmp_name'])) {
                if($_FILES['local_file']['type'] != 'text/xml') {
                    ...
                    try {
                        ...
                        $msg = __("File ").$_FILES['local_file']['name'].__(" loaded successfully.")."<br />".__("Click \"Save\" button to store new settings.");
                    }
                    catch(Exception$e) {
                        $error = TRUE;
                        $msg = $e->getMessage();
        ...
        case 'restore_defaults':
            try {
                ...
                $imported_defaults = $imported_config->getRelationShipSettings();
                $msg               = __('Default settings sucessfully restored.')."<br />".__("Click \"Save\" button to store new settings.");
            }
            catch(Exception$e) {
                $error = TRUE;
                $msg = $e->getMessage();
...
if(empty($msg)) {
    $msg = @$succ_msg;
    // FIXME: where does $succ_msg come from I wonder?
