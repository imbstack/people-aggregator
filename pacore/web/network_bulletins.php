...
if(!empty($_POST)) {
    ...
    $user = PA::$login_user;
    $value_to_validate = array(
        'title' => 'Title',
        'bulletin_body' => 'Bulletin body',
    );
    ...
    if(!$error_msg) {
        ...
        elseif(!empty($_POST['send_to_me_only'])) {
            // test send to admin user
            if(!$error_msg) {
                // if no errors
                $subject                                                                           = $_POST['title'];
                $bull_message                                                                      = $_POST['bulletin_body'];
                PANotify::send("bulletin_sent", $user, PA::$network_info, array('bulletin.message' => $bull_message, 'bulletin.subject' => $subject));
