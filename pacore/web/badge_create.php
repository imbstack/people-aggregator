...
class Badge {
        ...
        switch($op) {
            ...
            case 'enable':
                if(!isset($this->sections[$section])) {
                    return "invalid section for enable";
                }
                if(!isset($this->state[$section])) {
                    $this->state[$section] = array();
                }
            ...
            case 'disable':
                if(!isset($this->sections[$section])) {
                    return "invalid section for disable";
                }
                if(isset($this->state[$section])) {
                    $this->state[$section]['enabled'] = FALSE;
                }
                $this->save_state();
                return $this->render_section_inner($section);
            ...
            case 'exclude':
                ...
                return $this-> {
                    "render_".$section_singular."_image"
                }($subid);
                // change show_state
            case 'show':
    ...
    private function assert_post() {
        if($_SERVER['REQUEST_METHOD'] != "POST") {
            throw new PAException(OPERATION_NOT_PERMITTED, "This operation requires an HTTP POST");
        }
    }
    ...
    private function handle_op_show($section, $show_state) {
        if(!@$this->showing_states[$show_state]) {
            return "invalid show_state";
        }
...
ENS;
    ...
    function render_section_inner($key) {
        ...
        if($enabled) {
            ...
            elseif(in_array($key, array("friends", "friends_ext", "groups", "networks"))) {
                $selection_count = '(<span id="'.$key.'_count">'.count(@$section_state['included'])."</span> selected)";
            }
        }
        $inner = $enabled ? $this-> {
            "render_$key"
        }() : '<p class="badge_section_disabled">'.__("This section is disabled. Click the checkbox above to show it in your sidebar widget.").'</p>';
...
ENS;
    ...
    private function make_showing_link(&$state, $section_key, $qs) {
        ...
        // default
        $next_showing_state = $keys[(array_search($state['show'], $keys)+1)%count($keys)];
        // next one
...
function render_debug_sidebar() {
    ...
    ob_start();
    echo "<p>badges: ";
    var_dump($user->list_widgets());
    echo "</p>";
    try {
        $default_badge = $user->load_widget("default");
        echo "<p>default badge: ";
        var_dump($default_badge);
        echo "</p>";
    }
    catch(PAException$e) {
