...
class PAInstaller {
    ...
    private $steps = array(
        1 => array(
            'title' => 'License arrangement',
            'conf_section' => null,
            'curr_status' => false,
        ),
        2 => array(
            'title' => 'Installation requirements test',
            'conf_section' => null,
            'curr_status' => false,
        ),
        3 => array(
    ...
    private function handleGET() {
        ...
        if($step <= $this->curr_step) {
            // reset error flag because user click 'Back'
            $this->error = false;
        }
        $this->curr_step = $step;
    ...
    private function GET_step_3($params, $is_post = false) {
        ...
        if(!$is_post) {
            ...
            $form->addInputField('text', __('First Name'), array('id' => 'admin_first', 'required' => false, 'value' => (($is_post) ? $this->form_data['admin_first'] : trim($adm_data[3], "'\t "))));
            $form->addInputField('text', __('Last Name'), array('id' => 'admin_last', 'required' => false, 'value' => (($is_post) ? $this->form_data['admin_last'] : trim($adm_data[4], "'\t "))));
            $form->addInputField('text', __('Admin username'), array('id' => 'admin_username', 'required' => true, 'value' => (($is_post) ? $this->form_data['admin_username'] : "")));
            $form->addInputField('password', __('Admin password'), array('class' => 'admin_password', 'id' => 'admin_password', 'required' => true, 'value' => (($is_post) ? $this->form_data['admin_password'] : '')));
            $form->addInputField('text', __('Admin email'), array('id' => 'admin_email', 'required' => true, 'value' => (($is_post) ? $this->form_data['admin_email'] : "")));
            ...
            $form->addInputField('checkbox', __('Enable Multiple Networks'), array('id' => 'network_spawning', 'required' => true, 'value' => (($is_post) ? $this->form_data['network_spawning'] : 'checked'), "'\t "));
            ...
            $form->addInputField('text', __('Root Subdomain'), array('id' => 'subdomain', 'required' => true, 'value' => (($is_post) ? $this->form_data['subdomain'] : '')));
            $form->addHtml('</div>');
    ...
    private function GET_step_4($params) {
        ...
        $form->addInputField('text', __('Database name'), array('id' => 'db_name', 'required' => true, 'value' => ''));
        $form->addInputField('text', __('Database host'), array('id' => 'db_host', 'required' => true, 'value' => ''));
      ...
      foreach($results as $key => $data) {
        ...
        $form->addInputField('text', __('Database User Name'), array('id' => 'db_user', 'required' => true, 'value' => ''));
        $form->addInputField('password', __('Database password'), array('id' => 'db_password', 'required' => true, 'value' => ''));
        ...
        $form->addInputField('text', __('MySQL root username'), array('id' => 'mysql_root_username', 'required' => false, 'value' => ''));
        $form->addInputField('password', __('MySQL root password'), array('id' => 'mysql_root_password', 'required' => false, 'value' => ''));
    ...
    private function POST_step_4($params) {
      ...
      ";
        $data = array(
            'message' => array(
                'msg' => __('Please wait, trying to connect to the database.'),
                'class' => 'msg_warn',
            ),
    ...
    private function GET_step_5($params) {
        ...
        $this->updateSettings();
        $msg = "<p class='msg_info'>Congratulations. You have successfully installed People Aggregator.<br /><br />"."Administrator user name: <b>".$this->adm_data['login_name']."</b><br />"."Administrator password: <b>".$this->adm_data['password']."</b><br /><br />"."<b>For security reasons, change your initially assigned administrator password and be sure to delete your installation directory: \"pacore/web/install\"</b>. "."If you want to re-install People Aggregator application, make backup of your \"pacore/config/AppConfig.xml\" "."configuration file and delete it. Then reload page in your browser and installation process will run again.<br /><br />"."<center>Click <a href=\"".PA_BASE_URL.PA_ROUTE_HOME_PAGE."\"><b>here</b></a> to continue.</center></p>";
        $data = array(
            'message' => array(
                'msg' => $msg,
                'class' => 'msg_info',
    ...
    private function msg_unable_to_continue($params) {
       ...
       ";
        $data = array(
            'message' => array(
                'msg' => $msg.((!empty($params['message']['msg'])) ? "\nDetails: {$params['message']['msg']}" : ''),
                'class' => 'msg_err',
            ),
            'title' => $params['title'],
            'step' => $this->curr_step,
    ...
    private function checkDBSettings($params) {
        ...
        $dsn = "mysql://".$params['db_user'].":".$params['db_password']."@".$params['db_host']."/".$params['db_name'];
    }
