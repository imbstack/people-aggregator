...
function select_random_subset($rel_incl, $rel_show_what, $rel, $input_format = 'ids', $n_select = 8) {
    if($n_select < 2) {
        throw new PAException(OPERATION_NOT_PERMITTED, "select_random_subset() called with $n_select < 2");
    }
...
function badge_render_section($params) {
    ...
    foreach($sections as $section_name => $items) {
        if($section_name) {
            ...
            elseif(file_exists(PA::$core_dir."/web/images/".$img_leaf)) {
    ...
    if($show_what == 'show') {
        ...
        if(@$profile['enabled']) {
            ...
            if(count($prof)) {
                ...
                if($xp) {
                    $html .= "<p>School: ".htmlspecialchars($xp->query("name/text()")->item(0)->data.", ".$xp->query("year/text()")->item(0)->data).'</p>';
                }
                $xp = badge_parse_xml(@$prof['affiliations']);
                if($xp) {
                    $html .= "<p>Affiliations:
			".htmlspecialchars(@$xp->query("name/text()")->item(0)->data." ".@$xp->query("status/text()")->item(0)->data.", ".@$xp->query("year/text()")->item(0)->data).'</p>';
                }
            }
        }
        foreach(array(array("friends", "quote152", "internal"), array("friends_ext", "imported friends", "external"),) as $args) {
            ...
            if(@$friends['enabled']) {
                ...
                $max_friends = 8;
                // show max 8 friends in the widget
