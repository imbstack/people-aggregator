...
function mk_input($first, $last, $email, $checked = TRUE) {
    global $form_id;
    $input = '<input name="included[bc_'.$form_id.']" type="checkbox" '.($checked ? 'checked="checked"' : '').'"/>'.'<input type="hidden" name="firstname[bc_'.$form_id.']" value="'.htmlspecialchars($first).'"/>'.'<input type="hidden" name="lastname[bc_'.$form_id.']" value="'.htmlspecialchars($last).'"/>'.'<input type="hidden" name="email[bc_'.$form_id.']" value="'.htmlspecialchars($email).'"/>';
    ++$form_id;
    return $input;
}
if($_SERVER['REQUEST_METHOD'] != 'POST') {
    bc_err("This script only accepts POST requests.");
}
$bc_url      = trim(@$_REQUEST['bc_url']);
$bc_login    = trim(@$_REQUEST['bc_login']);
$bc_password = trim(@$_REQUEST['bc_password']);
if(strpos($bc_url, "http://") !== 0) {
    $bc_url = "http://$bc_url";
}
$parsed_url = @parse_url($bc_url);
if(!$parsed_url) {
    bc_err("Please enter a valid URL, e.g. mysite.grouphub.com", "basecamp_url");
}
$bc_url = $parsed_url['scheme']."://".$parsed_url['host'];
if(!$bc_login) {
    bc_err("Please enter your Basecamp login name", "basecamp_login");
}
if(!$bc_password) {
    bc_err("Please enter your Basecamp password", "basecamp_password");
}
...
try {
    switch(@$_REQUEST['op']) {
        ...
        // first, find all the company ids we can.
            ...
            $company_ids = array(
                (int) @$_REQUEST['project_company'] => @$_REQUEST['project_company_name'],
            );
            // - then scrape the contact list and add those in too
            foreach($bc->companies() as $company) {
                $cid = (int) $company['id'];
                if(!@$company_ids[$cid]) {
                    $company_ids[$cid] = $company['name'];
                }
            }
            $form_id = 1000;
            // start at 1000 so we don't overlap with the entered names/emails
