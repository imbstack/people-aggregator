    ...
    switch($op) {
        ...
        case 'download':
            ...
            if($f) {
                ...
                elseif(preg_match("|<m:human-readable.*?>(.*?)</m:human-readable>|s", $xml, $m)) {
                    $err = $m[1];
                }
                elseif(preg_match('|target-revision rev="(\d+)"|s', $xml, $m)) {
                    $target_rev = (int) $m[1];
                    if($target_rev == $rev) {
        ...
        case 'test-apply':
            ...
            echo '<div id="progress-detail"><h2>Verifying that the update can be installed.</h2>';
            flush();
            $msg_headers = array(
...
<ol>
 ...
 <li>
  ...
  <? show_op("soft-apply", "Click here to install the update without overwriting/deleting anything with local changes.");?>
            ...
            return;
            // don't show 'an update has been downloaded' text
        case 'apply':
            if(!file_exists($tmp_fn)) {
                break;
            }
        ...
        default:
            if($op) {
                throw new Subversion_Failure("Invalid operation: $op");
            }
            break;
    }
    if(file_exists($tmp_fn)) {
