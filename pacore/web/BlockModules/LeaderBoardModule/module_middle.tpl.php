...
// echo '<pre>'.print_r($users_ranking,1).'</pre>';
foreach($users_ranking as $ranked_user) {
    $even_odd = (++$i%2) ? "even" : "odd";?>
        <tr class="activities_ranking_row activities_ranking_<?=$even_odd?>">
          ...
          <td class="ranking_date">
            <?=(!empty($ranked_user->last_activity)) ? PA::datetime($ranked_user->last_activity, 'long', 'short')

    /* date("Y-m-d H:i:s", $ranked_user->last_activity) */ : __("unknown")?>
          </td>
