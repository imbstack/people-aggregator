...
class ShowContentModule extends Module {
    ...
    private function buildSearchStringArray($search_data, $request_data) {
        ...
        $is_date_range_search = (!empty($search_data["yFrom"]) || !empty($search_data["yTo"]) || !empty($search_data["mFrom"]) || !empty($search_data["mTo"]) || !empty($search_data["dFrom"]) || !empty($search_data["dTo"]));
    ...
    function render() {
        if($this->type == 'group') {
            ...
            if(PA::$login_uid) {
                ...
                $perm_params = array(
                    'permissions' => 'manage_groups, manage_roles',
                );
