...
class GroupInvitationModule extends Module {
    ...
    public function initializeModule($request_method, $request_data) {
        ...
        if($request_method == 'POST') {
            ...
            // temporrary - this page is not refactored yet
        }
    }
    ...
    */
    public function handlePOST_GroupInvitationSubmit($request_data) {
        if(isset($request_data['submit'])) {
            ...
            elseif(!empty($self_invite) || sizeof($invalid) > 0) {
    ...
    public function render() {
        ...
        for($i = 0; $i < $groups_count; $i++) {
            $this->user_groups[] = array(
                'gid' => $groups[$i]['gid'],
                'name' => stripslashes($groups[$i]['name']),
            );
        }
        $this->inner_HTML = $this->generate_inner_html(); $content = parent::render();
        return $content;
    }
