...
class ForumModule extends Module {
    ...
    function __construct() {
        ...
        $this->outer_template = "outer_public_forums_center_module.tpl";
        //'outer_public_center_module.tpl';
    }
    ...
    function initializeModule($request_method, $request_data) {
        ...
        elseif(!empty($this->shared_data['network_id'])) {
    ...
    function setSplashPage($request_data) {
        ...
        $this->inner_HTML = $this->generate_inner_html(array('forums_url' => $this->forums_url, 'theme_url' => $this->theme_url, 'allowed' => $allowed, 'user_msg' => $user_msg, 'url_src' => $url_src, 'message' => $this->message));
    }
    ...
    private function handleGET_reply($request_data, $quote = false) {
        global $error_msg;
        if(!($this->user_status&PaForumsUsers::_waiting) && !($this->user_status&PaForumsUsers::_limited) && !($this->user_status&PaForumsUsers::_banned) && ($this->user_status&PaForumsUsers::_allowed) || (($this->user_status&PaForumsUsers::_anonymous) && ($this->board_settings['allow_anonymous_post'] == true))) {
            ...
            if($reply_type == 'post') {
                ...
                if(is_object($post)) {
                    ...
                    $back_url = $this->get_url_for(array('thread_id' => $this->thread->get_id(), 'page' => $this->current_page, 'post_id' => $post->get_id()."#p_".$post->get_id()));
                }
                else {
                ...
                $back_url = $this->get_url_for(array('thread_id' => $this->thread->get_id(), 'page' => $this->current_page));
            }
            $this->set_inner_template('reply.tpl.php');
            $this->inner_HTML = $this->generate_inner_html(array('page_id' => $this->page_id, 'thread' => $this->thread, 'post' => $post, 'reply_type' => $reply_type, 'quote_post' => $quote, 'current_page' => $this->current_page, 'forums_url' => $this->forums_url, 'theme_url' => $this->theme_url, 'back_url' => $back_url, 'user_status' => $this->user_status, 'message' => $this->message, 'allow_anonymous' => $this->board_settings['allow_anonymous_post'], 'board' => $this->board, 'tiny_mce' => $this->tiny_mce));
        }
        else {
    ...
    private function handleGET_newTopic($request_data) {
        global $error_msg;
        if(!($this->user_status&PaForumsUsers::_waiting) && !($this->user_status&PaForumsUsers::_limited) && !($this->user_status&PaForumsUsers::_anonymous) && ($this->user_status&PaForumsUsers::_allowed)) {
            $action   = 'newTopic';
            $title    = __("New Thread");
            ...
            $this->inner_HTML = $this->generate_inner_html(array('forums_url' => $this->forums_url, 'theme_url' => $this->theme_url, 'title' => $title, 'action' => $action, 'fields' => $fields, 'back_url' => $back_url, 'message' => $this->message, 'board' => $this->board, 'tiny_mce' => $this->tiny_mce));
        }
        else {
    ...
    private function handleGET_newBoard($request_data) {
        ...
        if($request_data['owner_id'] == PA::$login_uid) {
            $params = array(
                "owner_id"    => $request_data['parent_id'],
                "network_id"  => $this->nid,
                "title"       => $title,
    ...
    private function handleGET_editSettings($request_data) {
        ...
        if(($this->user_status&PaForumsUsers::_owner) || ($this->user_status&PaForumsUsers::_admin)) {
            $tiny_mce_opt['options'] = array(
                "tinymce_rich" => "Rich TinyMCE",
                "tinymce_base" => "Base TinyMCE",
                "no_tinymce"   => "No TinyMCE",
            );
            $avatar_size_opt['options'] = array(
                "65x75"   => "Small",
                "85x100"  => "Medium",
                "110x130" => "Large",
            );
            ...
            $fields = array(
                'fields' => array(
                    array(
                        ...
                        'label'    => __('Forum name'),
                        'class'    => 'long_field',
                        'content'  => $this->board->get_title(),
                        'type'     => 'text',
                        'required' => true,
                    ),
                    array(
                        'name'     => 'description',
                        'label'    => __('Forum description'),
                        'class'    => 'textarea_field',
                        'content'  => $this->board->get_description(),
                        'type'     => 'textarea',
                        'required' => false,
                    ),
                    array(
                        'name'     => 'allow_anonymous_post',
                        'label'    => __('Allow anonymous post'),
                        'class'    => 'check_box',
                        'content'  => (($settings['allow_anonymous_post']) ? "checked" : null),
                        'type'     => 'checkbox',
                        'required' => true,
                    ),
                    array(
                        'name'     => 'allow_users_create_category',
                        'label'    => __('Allow users to create Categories'),
                        'class'    => 'check_box',
                        'content'  => ((@$settings['allow_users_create_category']) ? "checked" : null),
                        'type'     => 'checkbox',
                        'required' => true,
                    ),
                    array(
                        'name'     => 'allow_users_create_forum',
                        'label'    => __('Allow users to create Forums'),
                        'class'    => 'check_box',
                        'content'  => ((@$settings['allow_users_create_forum']) ? "checked" : null),
                        'type'     => 'checkbox',
                        'required' => true,
                    ),
                    array(
                        'name'     => 'membership_approval',
                        'label'    => __('Membership approval required'),
                        'class'    => 'check_box',
                        'content'  => (($settings['membership_approval']) ? "checked" : null),
                        'type'     => 'checkbox',
                        'required' => true,
                    ),
                    array(
                        'name'     => 'threads_per_page',
                        'label'    => __('Threads per page'),
                        'class'    => 'short_field',
                        'content'  => $settings['threads_per_page'],
                        'type'     => 'text',
                        'required' => true,
                    ),
                    array(
                        ...
                        'class'    => 'short_field',
                        'content'  => $settings['posts_per_page'],
                        'type'     => 'text',
                        'required' => true,
                    ),
                    array(
                    ...
                    array(
                        ...
                        'label'    => __('Edit mode'),
                        'class'    => 'medium_field',
                        'content'  => $tiny_mce_opt,
                        'type'     => 'select',
                        'required' => true,
                    ),
                    array(
                        'name'     => 'theme',
                        'label'    => __('Forum theme'),
                        'class'    => 'medium_field',
                        'content'  => $themes_list,
                        'type'     => 'select',
                        'required' => true,
                    ...
                    array(
                        'name'     => 'banned_message',
                        'label'    => __('Banned users message'),
                        'class'    => 'textarea_field',
                        'content'  => $settings['banned_message'],
                        'type'     => 'textarea',
                        'required' => false,
                    ),
                    array(
                        'name'     => 'join_requested_message',
                        'label'    => __('Join requested message'),
                        'class'    => 'textarea_field',
                        'content'  => $settings['join_requested_message'],
                        'type'     => 'textarea',
                        'required' => false,
                    ),
                    array(
                        'name'     => 'join_approved_message',
                        'label'    => __('Join approved message'),
                        'class'    => 'textarea_field',
                        'content'  => $settings['join_approved_message'],
                        'type'     => 'textarea',
                        'required' => false,
                    ),
                ),
    ...
    private function handleGET_edit($request_data, $edit_mode = 'edit') {
        ...
        if($edit_type == 'post') {
            $post = PaForumPost::getPaForumPost($request_data['post_id']);
            if(is_object($post)) {
                if((!$this->user_status&PaForumsUsers::_owner) && !($this->user_status&PaForumsUsers::_admin) && !((PA::$login_uid == $post->get_user_id()))) {
        ...
        elseif($edit_type == 'thread') {
            ...
            if((!$this->user_status&PaForumsUsers::_owner) && !($this->user_status&PaForumsUsers::_admin) && !((PA::$login_uid == $this->thread->get_user_id()))) {
                $error_msg = __("You don't have required permissions for this task!");
                return 'skip';
            }
            $back_url = $this->get_url_for(array('thread_id' => $this->thread->get_id(), 'page' => $this->current_page));
        ...
        $this->inner_HTML = $this->generate_inner_html(array('page_id' => $this->page_id, 'thread' => $this->thread, 'post' => $post, 'edit_type' => $edit_type, 'current_page' => $this->current_page, 'forums_url' => $this->forums_url, 'theme_url' => $this->theme_url, 'back_url' => $back_url, 'user_status' => $this->user_status, 'message' => $this->message, 'allow_anonymous' => $this->board_settings['allow_anonymous_post'], 'board' => $this->board, 'tiny_mce' => $this->tiny_mce));
    }
    ...
    private function handleGET_banUser($request_data) {
        ...
        if(($this->user_status&PaForumsUsers::_owner) || ($this->user_status&PaForumsUsers::_admin)) {
            ...
            try {
                ...
                $msg = __("User status sucessfully changed.")." ".__("User is")." ".(($is_banned) ? __("not")." " : null).__("banned now.");
                $this->redirectWithMessage($msg, $request_data, "info_message");
            }
            catch(Exception$e) {
    ...
    private function handlePOST_reply($request_data) {
        ...
        $user_id = ($this->user_status&PaForumsUsers::_anonymous) ?-1 // anonymous user : $this->forum_user->get_user_id();
        $parent_id = ($request_data['mode'] == 'post') ? $request_data['post_id'] : null;
        $content = $form_data['reply_content'];
        if($this->user_status&PaForumsUsers::_anonymous) {
            $content .= "<div class=\"edited_by\">\n".__("Posted by").": ".$form_data['anonymous_name'].", ".date("Y-m-d H:i:s")."\n</div>";
        }
        $params = array(
            "title"       => $form_data['reply_title'],
            "content"     => $content,
            "user_id"     => $user_id,
            "parent_id"   => $parent_id,
    ...
    private function handlePOST_editForum($request_data, $mode = 'edit') {
        ...
        if($mode == 'new') {
            $params = array_merge($params, array("is_active" => true, "category_id" => $request_data['category_id'], "sort_order" => null, "icon" => 'forum_default.gif', "created_at" => date("Y-m-d H:i:s")));
        }
        unset($request_data['action']);
        try {
            if($mode == 'new') {
    ...
    private function setupBoard(PaForumBoard$board) {
        ...
        switch($this->board_settings['post_edit_mode']) {
            case 'tinymce_rich':
                $this->tiny_mce = $this->getTinyMCE('base');
                break;
            case 'tinymce_base':
                $this->tiny_mce = $this->getTinyMCE('minimal');
                break;
            default:
                $this->tiny_mce = null;
        }
    ...
    private function getBoardThemes() {
        $themes_list = array();
        $theme_path = "web".DIRECTORY_SEPARATOR.PA::$theme_rel.DIRECTORY_SEPARATOR."forum".DIRECTORY_SEPARATOR."themes";
        $paths = array(
            PA::$core_dir.DIRECTORY_SEPARATOR.$theme_path,
    ...
    private function setBoardTheme($theme) {
        ...
        $this->theme_path = DIRECTORY_SEPARATOR.PA::$theme_rel.DIRECTORY_SEPARATOR."forum".DIRECTORY_SEPARATOR."themes".DIRECTORY_SEPARATOR.$theme;
