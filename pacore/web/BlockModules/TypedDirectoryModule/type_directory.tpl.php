...
<ul id="filters">
  ...
  foreach($mod->sortFields as $i => $field) {
    ?>
  <li <? if($mod->sort_by == $field['name']) {
        echo 'class="active"'?> ><a href="<?=PA::$url.PA_ROUTE_TYPED_DIRECTORY.$mod->directoryType?>?sort_by=<?=$field['name']?>"><?=$field['label']?></a></li>
  	<?
    }?>
</ul>
  <?php if(!empty($mod->page_links)) {?>
   <div class="prev_next">
     <?php if($mod->page_first) {
            echo $mod->page_first;
        }
    }?>
     <?php echo $mod->page_links?>
     <?php if($mod->page_last) {
        echo $mod->page_last;
    }?>
   </div>
  <?php
...
?>
			...
			<input type="submit" name="submit" value="<?=sprintf(__("Create new %s"), $createLabel)?>" />
		</form>
	</div>
</div>
<?php
...
$statesList = PA::getStatesList();
    ...
    foreach($mod->profilefields as $i => $field) {
        ...
        // display only fields that have value
        ?>
