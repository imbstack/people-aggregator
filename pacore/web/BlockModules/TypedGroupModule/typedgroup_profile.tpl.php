...
<?php if(!empty(PA::$login_uid)) {?>
	<form action="<?=PA::$url.PA_ROUTE_GROUP?>/action=<?=(empty($mod->is_member)) ? 'join' : 'update'?>&amp;gid=<?=$mod->group_details->collection_id?>">
	<?php if($mod->is_member) {?>
	...
	<div id="relationType" <? if($mod->is_member) {
	...
	<select name="relation"
        ...
        ?>
			<option value="<?=$k?>" <?php
			if($k == @$mod->relationType) {
            echo 'selected="selected"'?>><?=$v?></option>
			<?
        }
        ?>
	</select>
