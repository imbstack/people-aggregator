...
class FamilyModerationModule extends Module {
    ...
    private function handleGET_addChild($request_data) {
        ...
        $this->inner_HTML = $this->generate_inner_html(array('title' => $title, 'states' => $states, 'countries' => $countries, 'months' => $months, 'years' => $years, 'parent_uid' => PA::$login_uid));
    }
                ...
                try {
                    ...
                    foreach($profile_keys as $k => $pkey) {
                        if(!empty($_POST[$pkey])) {
                            ...
                            elseif(($pkey == 'country') && ($_POST[$pkey]) >= 0) {
                                $prof_rec = array(
                                    'uid'   => $newuser->user_id,
                                    'name'  => $pkey,
                    ...
                    if($user_joined) {
                        ...
                        if($type == 'child') {
                            ...
                            $_extra = serialize(array('user' => false, 'network' => false, 'groups' => array(
                                $this->family->collection_id,
                            )));
                            $user_roles[] = array(
                                'role_id' => CHILD_MEMBER_ROLE,
                                'extra' => $_extra,
                            );
    ...
    private function get_links() {
        ...
        foreach($members as $membersDetails) {
            if($membersDetails["user_type"] != 'owner') {
                ...
                $this->members_data[] = array(
                    'user_id'    => $membersDetails["user_id"],
                    'first_name' => $User->first_name,
                    'last_name'  => $User->last_name,
                    'email'      => $User->email,
                    'created'    => $membersDetails['join_date'],
                    'picture'    => $User->picture,
                    'user_type'  => $membersDetails["user_type"],
