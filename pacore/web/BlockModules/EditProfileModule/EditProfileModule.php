...
class EditProfileModule extends Module {
    ...
    public $valid_profile_types = array(
        'basic',
        'general',
        'personal',
        'professional',
    ...
    */
    public function basicProfileSave($request_data) {
        ...
        elseif(empty($request_data['email_address'])) {
            $this->message = __('Fields marked with * can not be empty, Email field is mandatory.');
        }
        elseif(!empty($request_data['pass']) || !empty($request_data['conpass'])) {
            ...
            elseif(strlen($request_data['pass']) < PA::$password_min_length) {
                $this->message = sprintf(__('Password should be of %s characters or more.'), PA::$password_min_length);
            }
            elseif(strlen($request_data['pass']) > PA::$password_max_length) {
