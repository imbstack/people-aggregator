  ...
  */
    ...
    */
    function __construct() {
        $this->do_skip = FALSE;
        if(PA::$profiler) {
            PA::$profiler->startTimer(get_class($this));
        }
    ...
    */
    function render() {
        ...
        //Module will be skipped if do_skip is true.
        $title               = $this->title;
        ...
        #modules
        if(empty($this->outer_template)) {
            throw new PAException(GENERAL_SOME_ERROR, 'Error in module; derived class must have $outer_template instance var');
        }
        ...
        if(PA::$profiler) {
            PA::$profiler->stopTimer(get_class($this));
        }
        return $contents;
        // Return the contents
    }
    ...
    function start_form($name_id, $method) {
        $ret = '<form method="'.$method.'" name="'.$name_id.'" id="'.$name_id.'" action="'.($this->post_url ? $this->post_url : "").'">';
    ...
    function input_tag($input_type, $name, $value) {
        return '<input type="'.$input_type.'" name="'.htmlspecialchars($this->param_prefix.$name).'" value="'.htmlspecialchars($value).'" />';
    }

    function textarea_tag($name, $value) {
        return '<textarea name="'.htmlspecialchars($this->param_prefix.$name).'">'.htmlspecialchars($value).'</textarea>';
    }
    ...
    public function setWebPageMessage() {
        if(!empty($this->message)) {
            if(!$this->isError) {
                ...
                $message = array(
                    'failure_msg' => NULL,
                    'success_msg' => $this->message,
                );
            }
            else {
                //Message is a failure message
                $message = array(
                    'failure_msg' => $this->message,
                    'success_msg' => NULL,
                );
            }
