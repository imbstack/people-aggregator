...
class MessageModule extends Module {
    ...
    */
    ...
    private function handleGET_view_message($request_data) {
        ...
        if((false === strpos($message_details['all_recipients'], PA::$login_user->login_name)) && ($this->folder_name == INBOX)) {
            $error_msg = __("You can't access other user's messages.");
            return;
        }
        if((false === strpos($message_details['sender_name'], PA::$login_user->login_name)) && ($this->folder_name == 'Sent')) {
            $error_msg = __("You can't access other user's messages.");
            return;
        }
        ...
        $this->inner_HTML = $this->generate_inner_html(array('prev_nxt_msg' => $prev_nxt_msg, 'message_details' => $message_details, 'mid' => $this->mid, 'folders' => $this->folders, 'folder_name' => $this->folder_name, 'current_theme_path' => PA::$theme_url, 'search_string' => $this->search_string));
    }
    ...
    private function handleGET_conversation_view($request_data) {
        ...
        $this->inner_HTML = $this->generate_inner_html(array('conversations' => $conversations, 'folders' => $this->folders, 'folder_name' => $this->folder_name, 'current_theme_path' => PA::$theme_url, 'search_string' => $this->search_string));
    }
