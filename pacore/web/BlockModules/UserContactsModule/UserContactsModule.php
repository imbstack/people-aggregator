...
class UserContactsModule extends Module {
    ...
    private $valid_cvs_mime_types = array(
    ...
    function initializeModule($request_method, $request_data) {
        ...
        switch($this->type) {
            ...
            break;
            case 'plaxo' : $this->contacts = $this->getUserContacts((int) PA::$login_uid, 'plaxo');
            break;
        case 'mslive' : $this->contacts = $this->getUserContacts((int) PA::$login_uid, 'mslive');
        break;
    case 'linkedin' : $this->contacts = $this->getUserContacts((int) PA::$login_uid, 'linkedin');
    break;
case 'outlook' : $this->contacts = $this->getUserContacts((int) PA::$login_uid, 'outlook');
break;
default :
}
$this->set_inner_template("contacts_main.tpl");
if(($request_method != 'POST') && ($request_method != 'AJAX')) {
    $this->inner_HTML = $this->generate_inner_html(array('type' => 'contacts', 'stype' => $this->type, 'message' => $invite_message, 'active_tab' => 1+array_search($this->type, $this->types), 'contacts' => $this->contacts));
}
}
...
private function getContactPicture($contact_extra) {
    ...
    elseif(!empty($contact_extra['extra']['business_photo'])) {
        // Plaxo contacts can have BusinessPhoto data field
        $img_url = $this->normalizeImgUrl($contact_extra['extra']['business_photo']);
        $picture_url = PA::$url.'/resize_img.php?src='.$img_url.'&height=98&width=98';
    }
    else {
