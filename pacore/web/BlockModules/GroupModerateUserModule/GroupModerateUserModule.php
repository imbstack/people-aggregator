        ...
        if(!empty($request_data["selectedArray"]) && !empty($request_data["group_id"])) {
            ...
            if(!empty($request_data["btn_approve"])) {
                for($counter = 0; $counter < count($selectedArray); $counter++) {
                    ...
                    $this->send_approval_message_to_user($selectedArray[$counter], $request_data["group_id"], true);
                    // "group join approved" message!
                }
            ...
            if(!empty($request_data["btn_deny"])) {
                for($counter = 0; $counter < count($selectedArray); $counter++) {
                    ...
                    $this->send_approval_message_to_user($selectedArray[$counter], $request_data["group_id"], false);
                    // "group join not approved" message!
                }
                $error_msg = __("User Denied");
            }
        }
        elseif(isset($request_data['btn_approve'])) {
            $error_msg = __("Please select a user for approval");
        }
        elseif(isset($request_data['btn_deny'])) {
