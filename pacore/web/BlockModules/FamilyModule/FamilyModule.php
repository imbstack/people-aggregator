...
class FamilyModule extends Module {
    ...
    function handleRequest($request_method, $request_data) {
        ...
        if(!empty($request_data['action']) && empty($request_data['module'])) {
            // when target module not defined!
    ...
    private function handleCreate($request_data) {
        ...
        if(empty($this->err)) {
            // create it
            TypedGroupEntity::sync(array('type' => $this->entity_type, 'name' => $this->entity_name, 'group_id' => $request_data['gid'],));
            // redirect
    ...
    private function handleGET_leave($request_data) {
        ...
        if(PA::$login_uid && !empty($this->shared_data['group_info'])) {
            ...
            elseif(Group::member_exists((int) $request_data['gid'], (int) PA::$login_uid)) {
                try {
