...
if(!empty($_POST) && !empty($_POST['btn_static_pages'])) {
    ...
    if(!$error) {
        try {
            ...
            $form_data = array();
            header("location: manage_static_pages.php?msg_id=$msg_id");
            exit;
        }
        catch(PAException$e) {
            $msg[] = $e->message;
...
elseif((!empty($_GET['do'])) && $_GET['do'] == 'edit' && !empty($_GET['id'])) {
    $edit = TRUE;
...
elseif((!empty($_GET['action'])) && $_GET['action'] == 'delete' && !empty($_GET['id'])) {
    if(!empty($_GET['id'])) {
        try {
            ...
            header("location: manage_static_pages.php?msg_id=12013");
            exit;
        }
        catch(PAException$e) {
            $msg[] = $e->message;
...
elseif(!empty($_GET['msg_id'])) {
    // message id was passed on URL
    $message = $_GET['msg_id'];
}
