    ...
    switch($_REQUEST['config_action']) {
            ...
            if(!empty($_FILES['local_file']['name']) && is_uploaded_file($_FILES['local_file']['tmp_name'])) {
                if($_FILES['local_file']['type'] != 'text/xml') {
                    ...
                    try {
                        ...
                        $error_msg                 = __("File ").$_FILES['local_file']['name'].__(" loaded successfully.")."<br />".__("Click \"Save\" button to save new settings.");
                    }
                    catch(Exception$e) {
                        $error = TRUE;
        ...
        case 'restore_defaults':
            try {
                ...
                $restore_settings          = $imported_config->getGeneralNetworkSettings();
                $restore_settings['extra'] = serialize($restore_settings['extra']);
                $error_msg                 = __('Default settings sucessfully restored.')."<br />".__("Click \"Save\" button to save new settings.");
            }
            catch(Exception$e) {
                $error = TRUE;
                $error_msg = $e->getMessage();
    ...
    if(!$error_post) {
        ...
        if(!empty($_FILES['inner_logo_image']['name'])) {
            ...
            if($file == false) {
                ...
                $data_icon_image = array(
                    'inner_logo_image' => $file,
                );
            }
        }
        else {
