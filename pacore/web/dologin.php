...
if(@$_POST['submit'] || @$_GET['action'] == 'login') {
    ...
    if($u > 0) {
        ...
        elseif($token_arr[0] == TRUE && $token_arr[1] == $_SESSION['user']['email']) {
            if($invitation_id) {
                // if token is valid
            ...
            if($group_invitation_id) {
                ...
                if(!empty($gid)) {
                    header("Location: ".PA_ROUTE_GROUP."/gid=$gid&action=join&GInvID=$group_invitation_id");
                }
                exit;
            }
