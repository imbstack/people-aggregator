...
if(!$error && ($do == 'approve')) {
    ...
    header("Location: ".PA::$url."/network_moderate_content.php?msg_id=1002");
    exit;
}
elseif(!$error && ($do == 'deny')) {
    Network::disapprove_content((int) $_GET['cid']);
    header("Location: ".PA::$url."/network_moderate_content.php?msg_id=1003");
    exit;
}
if(!empty($_POST['submit'])) {
    if(empty($_POST['cid'])) {
        header("Location: ".PA::$url."/network_moderate_content.php?msg_id=1007");
        exit;
    }
    switch($action) {
        case 'deny':
        ...
        case 'approve':
            ...
            break;
        default:
            header("Location: ".PA::$url."/network_moderate_content.php?msg_id=1006");
            exit;
    }
}
...
// end function
$page = new PageRenderer("setup_module", PAGE_NETWORK_MODERATE_CONTENTS, "Moderate Network Content", 'container_two_column.tpl', 'header.tpl', PRI, HOMEPAGE, PA::$network_info);
