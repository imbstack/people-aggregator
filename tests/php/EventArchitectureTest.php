...
class EventArchitectureTest extends PHPUnit_Framework_TestCase {
  ...
  public function testAddUpdateDeleteEvent() {
    ...
    $ea1->assoc_target_id = $user->user_id; // could very well be other user
    ...
    $network = Network::get_mothership_info(); // use the mothership
    // print_r($network);
    $ea2->assoc_target_type = 'network';
    $ea2->assoc_target_id = $network->network_id; // could very well be other user
    ...
    */
    ...
    $this->assertTrue(($a_cnt2 < $a_cnt), "expected " 
    . $a_cnt - 1
    . " assocs, got $a_cnt2\n");
    ...
    $this->assertEquals(
      date(DATE_ATOM, strtotime($e3->end_time)), 
      date(DATE_ATOM, $nextmonth));
    ...
    . "\n";
    $this->assertEquals(
      date(DATE_ATOM, strtotime($assocs[0]->end_time)), 
      date(DATE_ATOM, $nextmonth));

    echo "Testing if the EventAssociations now show up in Tomorrow's Calendar\n";
    echo "test range: " 
    . date(DATE_ATOM, $tomorrow) . " - " . date(DATE_ATOM, $tomorrow + $oneday) 
    . "\n";
    echo "event duration: " 
    . date(DATE_ATOM, strtotime($e3->start_time)) 
    . " - " 
    . date(DATE_ATOM, strtotime($e3->end_time)) 
    . "\n";
    // print_r($e3);
