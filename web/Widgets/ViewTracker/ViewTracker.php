...
Class ViewTracker {
  ...
  **/
  public function save() {
    ...
    try {
      ...
      $data = array($this->type, trim($this->title));
      ...
      if ($res->numRows()) {
        ...
        $data = array($this->view, $this->time_stamp, $this->id);
        Dal::query($sql, $data);
      } else {
        //insert the new view_count.
        $sql = 'INSERT INTO page_views (type, title, url, view, time_stamp) VALUES (?, ?, ?, ?, ?)';
        $data = array($this->type, trim($this->title), $this->url, 1, $this->time_stamp);
        Dal::query($sql, $data);
      }
    } catch (PAException $e) {
      throw $e;
  ...
  **/
  public function get_most_viewed_pages($duration = 7, $show='ALL',
$page=0, $direction = 'DESC', $sort_by = 'view', $cnt=FALSE) {
