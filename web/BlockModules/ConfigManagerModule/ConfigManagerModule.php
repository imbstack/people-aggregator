...
class ConfigManagerModule extends Module
{
        ...
        switch($request_method) {
          case 'GET' :
               ...
               $this->inner_HTML = $this->generate_inner_html(array('data' => $form,
                                                                    'mode_tag' => $this->mode_tag,
                                                                    'sect_tag' => $this->sect_tag,
                                                                    'message'  => null));
            break;
          case 'POST' :
            break;
    ...
    public function getConfigSection($config_sect, $manage_mode)
    {
        ...
        if ($manage_mode == 1) // super user mode
        {
            $condition = null;
        }
        list($info, $data) = $app->configObj->getConfigSection($section, $condition);//echo "<pre>" . print_r($data,1) . "</pre>";
        ...
        {
            $this->populate_data($item, $name, $form);//echo "<pre>" . print_r($item,1) . "</pre>";
        }
        ...
        if (is_array($item) && !empty($item['attributes']))
        {
            ...
            switch ($type )
            {
              ...
              case 'multi_array':
                ...
                $form->closeTag('div');
                break;
              default: throw new Exception('Unknown type of variable.');
            }
            array_pop($keys);
        }
        else
        {
            ...
            else
            {
                if ($parent_type)
                {
                    ...
                    switch ($type )
                    {
                      ...
                      case 'bool':
                        ...
                        $tag_name = "form_data[$this->section]" . implode('', $keys);
                        $tag_id = preg_replace('/[\[\]]+/', '_', implode('', $keys));
                        if (is_numeric($item)) // override length for numeric values
                        {
                           $len = 72;
                        }
                ...
                else
                {
                    $tag_name = "form_data[$this->section]" . implode('', $keys);
                    $tag_id = preg_replace('/[\[\]]+/', '_', implode('', $keys));
                    $len = (is_numeric($item)) ? 72 : 400;// override length for numeric values
                    $form->addContentTag('label', array('value' => ucwords(strtolower(str_replace('_', ' ', $name))), 'style' => 'display: block; color: #35558c'));
    ...
    private function recursive_update_data_object(&$value, $key) {
      ...
      static $keys = array();
        ...
        if(is_array($value)) {
          ...
          $path = implode("", $keys);
          $node_path = $keys;
          $node_key = trim(array_pop($node_path), "[]'"); // get node entry path
          ...
          if(eval("if(array_key_exists('$node_key', \$configData$node_root)) return true; return false;")) {
            ...
            if($attribs && isset($attribs['sub-type'])) {  // check for array of strings
              $var_value = explode("\n", $value);
              foreach($var_value as &$v_val) $v_val = trim($v_val);
            }
