...
<?php for($i=0;$i<count($links)-2;$i++) {?>   
  <li>
    <?php  // for Images and their hyperlinks
       if ( $links[$i]['type'] == 7) {
         $link_image = "<img src='".$current_theme_path."/images/sb-70X50.jpg' width='70px' height='38px' border='0' alt='SB'/>";
         $hyper_link = PA::$url . PA_ROUTE_CONTENT . '/cid='.$links[$i]['content_id'];
       }
       else {
         $link_image = "<img src='".$current_theme_path."/images/audio-icon.gif' border='0' alt='PA'/>";
    ...
    ?>         
     <p class="choose">
       ...
       <a href="#"><img src="<?php echo $current_theme_path;?>/images/16_delete.gif" alt="" height="16" width="16" border="0" title="Delete" onclick="edit_delete_media('<?=$links[$i]['content_id']?>','delete','image_upload1');"></a>
    </p>
    
    <? } if (isset($_GET['gid']) && !empty ($_GET['gid']) && ($links[$i]['author_id'] == $login_uid )) { // Handling Groups Media Gallery 
       if($links[$i]['type'] == 7) {
          $link_for_editing = PA::$url . "/post_content.php?cid=".$links[$i]['content_id'];
       } else {
         $link_for_editing = PA::$url . "/edit_media.php?uid=".$login_uid."&amp;cid=".$links[$i]['content_id']."&amp;type=Audios";
       }
    ?>
     <p class="choose">
       ...
       <a href="#"><img src="<?php echo $current_theme_path;?>/images/16_delete.gif" alt="" height="16" width="16" border="0"  onclick="edit_delete_media('<?=$links[$i]['content_id']?>','delete','image_upload1');"></a>
    </p>
