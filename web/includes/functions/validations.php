...
function get_valid_filter_arrays() {
  $tags = array('strong', 'ol', 'ul', 'li', 'b', 'i', 'em', 'strike', 'u', 'p', 'img');
  $attr = array('href','src','width','height','alt');
...
function check_file_type( $file_path, $file_type='image' ) {
    $return = '';
    if( !file_exists( $file_path ) ) { // file path specified is not correct.
      $return = 'Please specify a valid file name';
      return $return;
    }
    
    $mime_type = @exec('file -bi '.$file_path);// checking the mime type of a file
    ...
    $valid_extensions = explode( '|', $GLOBALS['file_type_info'][$file_type]['ext'] );//getting the valid extensions from config.inc
