...
require_once "pacore/web/includes/classes/FormHandler.php";
...
function check_session($login_required=TRUE, $redirect_function=NULL) {
  ...
  if ($not_logged_in) {
    ...
    //
    // NOTE: This part of code is obsolete now! It should be removed.  (note by Zoran Hron)
    //
    if ($login_required) {
 ...
 **/
        ...
        if( !$page_access ) {
          ...
          else if($user_action == 'leave network') {
            $redirect_url = NULL;
          }
          else if( empty( $user_type ) || $user_type == NETWORK_WAITING_MEMBER) {
            if($user_action == 'leave network') {
              unset($_SESSION['user']['action']);
              $redirect_url = NULL;
            } else {
              $redirect_url = $redirect_page . "?msg=7002";
            }
          } else {
            $redirect_url = NULL;
          }
