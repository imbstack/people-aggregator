...
class SimplePlaxoClient extends ServiceProtocolHandler {
 ...
 public function __construct($proto_ver = 1, $client_name = 'Dev Clientname/1.0',
                             $client_os = 'linux/PHP', $client_platform = 'PlaxoThunderBird/0.9') {
...
//      echo "Mapped: <pre>" . print_r($this->mapped_contacts, 1) . "</pre>";
 ...
 public function parseResponseData($buffer) {
    ...
    if(isset($matches[0]) && (count($matches[0]) > 0)) {
      foreach($matches[0] as $line) {
        ...
        $ret_data[] = array('tag' => $tag_name, 'fields' => $data_arr);
      }
    }
    return $ret_data;
 }
 ...
 private function formatPlaxoTag($tag_name, $params = array(), $tag_content = null) {
    ...
    $tag .= "]%0a";
    if($tag_content) $tag .= $tag_content;
    $tag .= "['/" . $tag_name . "']";
    return $tag;
 }
