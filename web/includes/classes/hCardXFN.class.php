...
class hCardXFN  {
  ...
  protected $vcard_types    = array( 'personal', 'professional');
  protected $relation_types = array( 'internal', 'flickr', 'facebooks', 'others' );

  
  protected $root_classes = array( 
		'org', 'tel', 'geo', 'fn', 'n', 'adr', 'email', 'label', 'bday', 'agent',
    'nickname', 'photo', 'class', 'category', 'key', 'logo', 'mailer', 'note',
    'tz', 'uid', 'url', 'rev', 'role', 'sort-string', 'sound', 'title', 'include'
  );
  
  protected $sub_classes = array( 
		'org'   => array('organization-name', 'organization-unit', 'type'),
    ...
    'note'  => array('languages', 'awards', 'religion', 'ethnicity', 'political', 'passion', 'activities',
                     'books', 'movies', 'music', 'tv', 'food', 'summary', 'college', 'degree')
  ); 
  ...
  private $attributes_map = array(
		...
		'type'	  => array('ABBR' => 'title'),
		'include' => array('A' => 'href', 'OBJECT' => 'data'),
  	'key'     => array('OBJECT' => 'type'),
  	'sound'   => array('OBJECT' => 'data'),
...
//-------------- Public functions -----------------------------------------------------------------------------------------
  ...
  public function getFromString($strHtml) {
    ...
    $vcards = @$xpath->query("//*[contains(@class,'vcard')]");                       // look up for hCards
    if(($vcards->length) == 0) {
    ...
    foreach ($vcards as $vcard) {
      ...
      if(((count($vcard_class) > 0) && (isset($vcard_class[1]))) 
                                    && ((in_array($vcard_class[1], $this->vcard_types))
                                    || (in_array($vcard_class[1], $this->relation_types)))) {
        $this->outData[$cnt]['type'] = $vcard_class[1];                             // store type of the current hCard
      } else {
        $this->outData[$cnt]['type'] = 'personal';                                  // type not defined, we will use 'personal' as default
      }
  ...
  public function getFromURL($url, $tidy = false) {
    $this->url = $url;
	  if ($tidy){
		  $url = TIDY_URL . $url;                                                    // use Tidy proxy service to clean HTML
	  }                                                                              
  ...
  public function getFromFile($fname) {                                          // file name with full path
  ...
  public function getCardsByType($type) {
    $cards = array();
    if(!in_array($type, $this->vcard_types)) return $cards;              // unknown type: return empty array
    foreach($this->outData as $k => $v) {
      if(($v['type'] == $type) && (!isset($v['rel_type'])))  $cards[$k] = $v;   
    }
    return $cards;
  }
...
//-------------- Private functions --------------------------------------------------------------------------------------
  ...
  private function parseNode($node, $classes) {
    $r_data = array();
    $n_attr = false;
	for ($i=0; $i < count($classes); $i++){                                         // iterate trough all valid class names
		$xpath	 = ".//*[contains(concat(' ',normalize-space(@class),' '),' " . $classes[$i] . " ')]";
		$results = $node->xpath($xpath);
        foreach($results as $result) {
          $class_names =  explode(" ", $result['class']);                           // class attribute contains more than one class name?
          foreach($class_names as $class_name) {
            $n_attr = false;
            if(array_key_exists($class_name, $this->attributes_map)) {
               $n_attr = $this->getAttributeValue($result, $class_name);            // look up for attributes     
               if($n_attr && ($class_name != "include")) $r_data[$class_name] = $n_attr;
            }
            if(isset($this->sub_classes[$class_name])) {
              $sclasses = $this->sub_classes[$class_name];                          // check node for sub-classes
              $tmp_arr = $this->parseNode($result, $sclasses);                      // recursive call -
              if(isset($r_data[$class_name]) && is_array($r_data[$class_name])) {   
                 if(count($tmp_arr)>0) {
                   ...
                   array_push($r_data[$class_name], trim(preg_replace('/[\r\n\t]+/', ' ', 
                    ...
                    if(count($tmp_arr)>0) {
                       if($n_attr) array_push($tmp_arr, $n_attr);                                        
              ...
              if(!($class_name == 'include')) {
                 if($n_attr == false) $r_data[$class_name] = trim(preg_replace('/[\r\n\t]+/', ' ', implode(' ', $result->xpath('child::node()'))));
              } else {
                $include_arr = $this->getAttributeValue($result, $class_name);                       // handle includes
                if (is_array($include_arr)) $r_data = array_merge($r_data, $include_arr); 
              }  
            }  
          }
        }
  ...
  private function getAttributeValue($node, $class_name) {
	...
	if ($tag_name == 'DEL') return $r_value;                                       // ignore DEL tags

    if(($tag_name == 'A' || $tag_name == 'OBJECT') && $class_name == 'include') {  // check for include patterns
	  return $this->handle_includes($node, $tag_name, $class_name); 
    }

    if(($tag_name == 'OBJECT' && ($class_name == 'key' || $class_name == 'sound'))) { // check for key or sound pattern
       $r_value['value'] = trim(preg_replace('/[\r\n\t]+/', ' ', implode(' ', $node->xpath('child::node()')))); 
       if(isset($node['type'])) $r_value['type'] =  trim((string)$node['type']);
       if(isset($node['data'])) $r_value['data'] =  trim(preg_replace('/[\r\n\t\s]+/', '', (string)$node['data']));
       return $r_value;
    }
          
    if(($tag_name == 'A' && $class_name == 'url') && isset($node['rel'])) {        // check for XFN relations 
      if(($node['rel'] != 'me') && ($node['rel'] != strtoupper('me')))
        $this->current_card['rel_type'] = trim((string)$node['rel']);
    }
			
    if(($class_name == 'current') || ($class_name == 'prior')) {                   // check for organization type 
        $this->current_card['org']['type'] = $class_name;
    }
           
           
    if(isset($this->attributes_map[$class_name][$tag_name])) {                     // look up attributes map values
      $nattr = $this->attributes_map[$class_name][$tag_name]; 
      if(isset($node[$nattr]))  $r_value = trim((string)$node[$nattr]);
    }

    if($this->url && in_array($class_name, $this->resolve_classes)) {              // parsing data from URL, maybe we need 
        $r_value = $this->resolveURLs($r_value);                                   // resolve URL
    }

    if($class_name == 'email') {                                                   
     	$parts	= parse_url($r_value);                                               // resolve email address
		$r_value = $parts['path'];
    }
  ...
  private function &handle_includes($node, $tag_name, $class_name) {
    if(isset($this->attributes_map[$class_name][$tag_name])) {                     // look up attributes map 
      $tmp_arr = array();
      $nattr = $this->attributes_map[$class_name][$tag_name]; 
      if(isset($node[$nattr])) {
         $id = str_replace('#', '', (string)$node[$nattr]);
		     $xpattern	= "//*[@id='$id']";
         $xpath = new DOMXPath($this->domXML);
         $includes = $xpath->query($xpattern);                                     // look up for includes 
		     foreach ($includes as $include){
           $xml_str = '<nodes>
                         <node>'.
                           $this->domXML->saveXML($include).
                        '</node>
                       </nodes>';
  ...
  private function resolveURLs($value) {
     $r_val = $value;
     if(strpos($value, "://") === false) {                          // is relative URL?
  ...
  private function do_n_from_fn(&$vcard)
  {
	if (array_key_exists('fn', $vcard) &&  !is_array($vcard['fn']) 
                                       &&  !array_key_exists('n', $vcard)
                                       && (!array_key_exists('org', $vcard) || $vcard['fn'] != $vcard['org'])) 
    {
	  $n_patterns = array( array('/^(\S+),\s*(\S{1})$/', 2, 1),     // Lastname, Initial
	                array('/^(\S+)\s*(\S{1})\.*$/', 2, 1), 	        // Lastname Initial(.)
	                array('/^(\S+),\s*(\S+)$/', 2, 1), 		          // Lastname, Firstname
	                array('/^(\S+)\s*(\S+)$/', 1, 2) 		            // Firstname Lastname
    );  
  ...
  {
     if (array_key_exists('n', $vcard) && is_array($vcard['n']) 
                                       && (!array_key_exists('fn', $vcard) || !isset($vcard['fn']) )  
                                       && (!array_key_exists('org', $vcard) || $vcard['fn'] != $vcard['org'])){
  ...
  {
     if (array_key_exists('note', $vcard)) {            // map all unamed notes into 'summary' field
