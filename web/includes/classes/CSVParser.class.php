...
//    echo "<pre>" . print_r($this->contacts, 1) . "</pre>";
  ...
  private function formatData($data_str) {
    ...
    $keys = array_shift($data);                   // first element contains field keys
    $out_data = array();
    foreach($data as $k => $v) {
  ...
  private function getCSVSeparator($csv_lines) {
    $sep_weight = array();
     ...
     if(!empty($sep_idx) && is_numeric($sep_idx)) {
       $separator = $this->valid_separators[$sep_idx];  // separator found!
     } else {
       $separator = $this->valid_separators[0];  // return default CSV separator!
     }
     return $separator;
  }
 ...
 protected function getInviteMails($contacts) {
   ...
   foreach($contacts as $contact) {
      ...
      if(!empty($contact['personal']['email'])) {
        ...
        $type  = 'personal';
      } else if(!empty($contact['professional']['email'])) {
      ...
      if(!empty($contact['personal']['first_name']) || !empty($contact['personal']['last_name'])) {
        $name = null;
        if(!empty($contact['personal']['first_name'])) $name  = $contact['personal']['first_name'] . ' ';
        if(!empty($contact['personal']['last_name']))  $name .= $contact['personal']['last_name'];
      } else if(!empty($contact['extra']['contact_identifier'])) {
        $name = $contact['extra']['contact_identifier'];
      } else if(!empty($contact['extra']['e-mail_display_name'])) {
        $name = $contact['extra']['e-mail_display_name'];
      } else if(!empty($contact['professional']['company'])) {
