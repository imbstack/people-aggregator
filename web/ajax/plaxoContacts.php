...
/**
 *
 * @name getPlaxoContacts.php
 * @author Zoran Hron
 * @version    0.2.1
 *
 *
 * @brief : read user contacts from Plaxo service
 *
 **/
  ...
  switch($action) {
    case 'login':
            if(!empty($pUID) && !empty($pPSW)) {
               $plx1 = new SimplePlaxoClient();
               if(!$plx1->connectService(array('user_id' => $pUID, 'password' => $pPSW))) {
                 $hasErrors = true;
                 $msg = $plx1->getLastError();
               } else if(false !== ($cntcts = $plx1->getPlaxoContacts('PlaxoDataMapper', true))) {
                  if(count($cntcts) > 0)
...
<div id="pl_response" class="invite_container">
    ...
    <div id="pl_messages" class="invite_message">
      <?php if($msg) { ?>
        <?= $msg ?>
      <?php } ?>
    </div>
    ...
    <form name="plaxo_invite_form" id="plaxo_invite_form" action="" method="post">
      <input type="hidden" name="ajax_submit" value="submit" />
      <input type="hidden" name="import_type" value="plaxo" />
      <input type="hidden" name="action" value="import_contacts" />
      <input type="hidden" name="contacts_encoded" value="<?= base64_encode(serialize($plx1->mapped_contacts)) ?>" />
