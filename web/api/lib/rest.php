...
function handle_rest_call()
{
    ...
    while (sizeof($path) && !$path[0]) array_shift($path);
    ...
    foreach ($args_desc as $arg_name => $arg_desc) {
        // get value
        $v = @$_REQUEST[$arg_name];
        if ($v === NULL) continue; // missing keys will be detected during validation
        ...
        case 'string':
	case 'enum':
	  if ($magic) $v = stripslashes($v); break;
        case 'boolean':
            switch ($v) {
            ...
            break;
        case 'int': $v = intval($v); break;
        case 'float': $v = floatval($v); break;
    ...
    try {
      $ret = $php_func($args);
      // check output type
      if (gettype($ret) != 'array') api_error("Returned data from $php_func should be an array, but received ".gettype($ret)." instead.");
      $validate = TRUE;
      if ($path[0] == 'peopleaggregator') {
          if (!array_key_exists("success", $ret)) api_error("Missing 'success' field in returned data from $php_func.");
          if (!$ret['success']) $validate = FALSE;
      }
      // check output
      if ($validate) validate_content($ret, $func_desc['return'], "REST response", "auto", true);
    } catch (PAException $e) {
