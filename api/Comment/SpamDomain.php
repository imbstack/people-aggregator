...
class SpamDomain {
  ...
  public function __construct($host, $noisy=FALSE) {
    if (!trim($host)) throw new PAException(BAD_PARAMETER, "Invalid host");
	...
	$this->$k = $v;
      ...
      if ($noisy) echo "Querying blacklists for $host:";
  ...
  public function find_associated_domains() {
    $sth = Dal::query(
      "SELECT sd.domain domain, dic2.domain_id domain_id, COUNT(dic2.domain_id) match_count
      ...
      GROUP BY dic2.domain_id ORDER BY match_count DESC",
      array($this->id));
    return Dal::all_assoc($sth);
  }
