...
class ShadowUser extends User {
  ...
  function load($remote_id_or_userinfo) {
    ...
    $u = 
      parent::quick_search_extended_profile(
        'user_id',
        $remote_id, 
        $this->namespace);
    try {
    ...
    $this->display_login_name =
      $this->get_profile_field($this->namespace, 'display_login_name');
    return $this->user_id;
  }
  ...
  public function sync($userdata) {
    ...
    foreach ($extendeddata as $key=>$data) {
      if (isset($userdata[$key])) { // only if it got passed
  ...
  static function create($namespace, $userinfo, $network_info) {
    ...
    $userinfo['confirm_password'] = $userinfo['password'] = 
      substr( md5($userinfo['email'] . rand()), 0, 12 ); 
